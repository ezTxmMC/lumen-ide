...
export function applyTextEdits(text: string, edits: TextEdit[]): string {
  ...
  let out = text
  for (const e of ordered) out = out.slice(0, e.from) + e.insert + out.slice(Math.max(e.from, e.to))
  return out
}
...
function collect(edit: WorkspaceEdit) {
  ...
  for (const change of edit.documentChanges ?? []) {
    if ('kind' in change) {
      if (change.kind === 'create') ops.push({ kind: 'create', path: uriToPath(change.uri) })
      if (change.kind === 'delete') ops.push({ kind: 'delete', path: uriToPath(change.uri) })
      if (change.kind === 'rename') ops.push({ kind: 'rename', from: uriToPath(change.oldUri), to: uriToPath(change.newUri) })
      continue
    }
...
export async function applyWorkspaceEdit(edit: WorkspaceEdit, label?: string): Promise<boolean> {
  ...
  if (touched && label) store.notify(label, 'success')
  return true
}
