...
function wireLogs() {
  ...
  const forward = (entry: { jobId: string; text: string }) => {
    if (entry.jobId !== currentJob) return
    log(entry.text)
  }
...
export function closeLspInstall(decline = false) {
  if (lspInstall.isBusy()) return
  const languageId = state.request?.languageId
  if (decline && languageId) useStore.getState().declineLspInstall(languageId)
  update(idle())
}
...
async function installOne(target: InstallTarget, jobId: string, secret: { value: PasswordAnswer | null }): Promise<Outcome> {
  ...
  if (!command) return { ok: false, message: t('lsp.dialog.noCommand') }
...
async function restartFor(configs: LspConfig[]) {
  ...
  for (const tab of store.tabs) {
    if (!tab.path || tab.virtual || seen.has(tab.path)) continue
    const spec = store.languageFor(tab)
    if (!spec?.lsp?.some((config) => commands.has(config.command))) continue
    seen.add(tab.path)
...
export async function startInstall(targets: InstallTarget[]) {
  if (lspInstall.isBusy() || !targets.length) return
  wireLogs()
  cancelled = false
  ...
  for (const [index, target] of targets.entries()) {
    ...
    if (outcome.ok) installed.push(target.config)
    if (!outcome.ok && outcome.message) log(outcome.message)
    update({ results: [...results] })
  }
  secret.value = null
  currentJob = null

  if (installed.length) await restartFor(installed)
  ...
  if (done.length) store.notify(t('run.installed', { name: done.join(', ') }), 'success')
  if (failed.length) store.notify(t('run.installFailed', { name: failed.join(', ') }), 'error')
...
export function cancelInstall() {
  if (!lspInstall.isBusy()) return
  cancelled = true
  if (passwordWaiter) answerPassword(null)
