...
export async function rememberOpenFiles(get: Get, set: Set, root: string) {
  ...
  if (s.workspace === root) set({ projectConfig: next })
...
export async function restoreSession(get: Get, set: Set, session: NonNullable<WorkspaceDef['session']>, newGroupId: () => string) {
  ...
  for (const path of (first ?? []).slice(0, 24)) await get().openFile(path).catch(() => {})
  if (second?.length) {
    ...
    for (const path of second.slice(0, 24)) await get().openFile(path).catch(() => {})
  }
