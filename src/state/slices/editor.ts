...
interface Ctx {
  get: () => State;
  set: Parameters<Slice<EditorSlice>>[0];
  access: StoreAccess;
  commit: (groups: EditorGroup[], activeGroupId: string, options?: { remember?: boolean; }) => void;
  patchTab: (id: string, patch: Partial<Tab>) => void;
}

async function applyFsChange({ get, patchTab }: Ctx, change: FsChange) {
...
function insertTab({ get, set, commit }: Ctx, tab: Tab): boolean {
  const s = get();
  const twin = s.tabs.find((open) => open.path === tab.path);
  if (twin) {
    if (!tab.preview && twin.preview) {
      get().pinTab(twin.id);
    }
...
async function openViewer(ctx: Ctx, path: string, viewer: MediaKind, preview: boolean) {
  const { get } = ctx;
  try {
...
/** Opens a file in a tab: focuses an existing one, hands it to an extension or viewer, or reads it as text. */
async function openFileTab(ctx: Ctx, path: string, preview: boolean) {
  const { get } = ctx;
    ...
    if (await claimFileOpen(path)) {
      return;
    }

    const kind = mediaKindForPath(path);
    if (kind) {
      await openViewer(ctx, path, kind, preview);
      return;
    }
...
/** Switches an SVG tab between its image viewer and its text. */
async function setTextMode(ctx: Ctx, id: string, text: boolean) {
  const { get, patchTab } = ctx;
    const tab = get().tabs.find((open) => open.id === id);
    if (!tab?.path || !isSvgPath(tab.path)) {
      return;
    }
    if (!text) {
    ...
    if (fresh?.path) {
      void lsp.openDocument(get().languageFor(fresh), fresh.path, content);
    }
}

function openActions(ctx: Ctx): Pick<EditorSlice, 'openFile' | 'setTabTextMode' | 'openAt' | 'openVirtual' | 'consumeReveal' | 'newFile'> {
  const { get, set, access } = ctx;
  return {
    openFile(path, preview = false) {
      return openFileTab(ctx, path, preview);
    },

    setTabTextMode(id, text) {
      return setTextMode(ctx, id, text);
    },
...
function tabActions(ctx: Ctx): Pick<EditorSlice, 'closeTab' | 'closeTabEverywhere' | 'closeOthers' | 'closeAll' | 'setActiveTab' | 'cycleTab' | 'reopenClosedTab'> {
  const { get, set, commit } = ctx;
  return {
    closeTab(id, groupId) {
      const s = get();
...
function groupActions(ctx: Ctx): Pick<EditorSlice, 'moveTab' | 'splitEditor' | 'unsplitEditor' | 'focusGroup' | 'focusNextGroup' | 'moveTabToOtherGroup' | 'setSplitRatio'> {
  const { get, set, commit } = ctx;
  return {
    moveTab(tabId, fromGroupId, toGroupId, index) {
      ...
      const groups = s.groups.map((g) => {
        if (g.id === from.id && g.id === to.id) {
          return reorder(g, tabId, index);
        }
        if (g.id === from.id) {
          return withoutTab(g, tabId);
        }
        if (g.id === to.id) {
          return withTab(g.tabIds.includes(tabId) ? g : insertAt(g, tabId, index), tabId);
        }
        return g;
      });
...
function pathActions(ctx: Ctx): Pick<EditorSlice, 'retargetTab' | 'pathRenamed' | 'pathDeleted'> {
  const { get, set, patchTab } = ctx;
  return {
    retargetTab(id, path) {
      const language = matchLanguage(path, registry.languages());
      set((s) => ({
    ...
    pathRenamed(from, to) {
      ...
      for (const tab of get().tabs) {
        ...
        if (tab.path !== from && !tab.path.startsWith(prefix)) {
          continue;
        }
        ...
        if (moved && !(moved.viewer && moved.readonly)) {
          void lsp.openDocument(get().languageFor(moved), next, moved.content);
        }
      }
      scheduleOpenFilesSync(get, set);
    },
...
function diskActions(ctx: Ctx): Pick<EditorSlice, 'reloadTab' | 'handleFsChanges' | 'syncTabsWithDisk'> {
  const { get, patchTab } = ctx;
  return {
    async reloadTab(id) {
      const tab = get().tabs.find((open) => open.id === id);
      ...
      if (tab.viewer) {
        patchTab(id, { revision: (tab.revision ?? 0) + 1, missing: false });
      }
      if (tab.viewer && tab.readonly) {
        return;
      }
      let content: string;
      let eol: LineEnding;
      ...
      if (editorBridge.tabId !== id) {
        lsp.changeDocument(tab.path, content);
      }
    },

    async handleFsChanges(changes) {
      for (const change of changes) {
        await applyFsChange(ctx, change);
      }
    },
...
function contentActions(ctx: Ctx): Pick<EditorSlice, 'updateContent' | 'pinTab' | 'setCursor'> {
  const { get, set, patchTab } = ctx;
  return {
    ...
    setCursor(cursor) {
      ...
      if (current.line === cursor.line && current.character === cursor.character) {
        return;
      }
      set({ cursor });
    },
...
function saveActions(ctx: Ctx): Pick<EditorSlice, 'saveTab' | 'saveAll'> {
  const { get, set, patchTab } = ctx;
  return {
    async saveTab(id) {
      const s = get();
      ...
      if (wasNew) {
        void lsp.openDocument(language, target, fresh.content);
      }
      if (!wasNew) {
        lsp.saveDocument(target, fresh.content);
      }
      ...
      if (isProjectConfigPath(target)) {
        void get().reloadProjectConfig();
      }
    },
...
function lookupActions(ctx: Ctx): Pick<EditorSlice, 'activeTab' | 'languageFor' | 'languages'> {
  const { get } = ctx;
  return {
    activeTab() {
      const s = get();
    ...
    languageFor(tab) {
      ...
      if (tab.languageId) {
        return registry.languages().find((language) => language.id === tab.languageId) ?? null;
      }
      if (!tab.path || tab.virtual) {
        return null;
      }
...
export const createEditorSlice: Slice<EditorSlice> = (set, get) => {
  ...
  const commit: Ctx['commit'] = (groups, activeGroupId, options) => commitGroups(access, groups, activeGroupId, options);
  const patchTab: Ctx['patchTab'] = (id, patch) =>
    set((s) => ({ tabs: s.tabs.map((tab) => (tab.id === id ? { ...tab, ...patch } : tab)) }));
  const ctx: Ctx = { get, set, access, commit, patchTab };
