...
import type { EditorGroup, RecentProject, Slice, WorkspaceDef, WorkspaceSlice } from '../types'
...
async function projectServerLanguages(root: string, kindLanguages: string[]): Promise<string[]> {
  ...
  if (kindLanguages.length) return kindLanguages.filter((id) => counts.has(id) && withServer(id))
...
export const createWorkspaceSlice: Slice<WorkspaceSlice> = (set, get) => {
  const remember = (root: string) => rememberOpenFiles(get, set, root)
  const capture = () => captureSession(get, set)

  /** Start the chosen language servers of the project, before any file is open. */
  async function startProjectServers(root: string) {
    const s = get()
    if (!s.effects.lsp || !s.effects.lspAutoStart || s.workspace !== root) return
    ...
    for (const id of languages) {
      ...
      if (spec && get().workspace === root) void lsp.startForProject(spec, s.project?.root ?? root)
    }
  }

  /** Open the files remembered in the project configuration, into two groups where it was split. */
  async function restoreOpenFiles(root: string, config: ProjectConfig) {
  ...
  return {
    ...
    async setWorkspace(root, options = {}) {
      ...
      void startProjectServers(root)

      if (options.restoreFiles === false || !get().effects.restoreOpenFiles || get().tabs.length > 0) return
      await restoreOpenFiles(root, config)
    },

    async closeWorkspace() {
      const previous = get().workspace
      if (previous) await remember(previous)
      capture()
      get().closeAll()
    ...
    async openWorkspace(id) {
      ...
      if (target.session) await restoreSession(get, set, target.session, nextGroupId)
      get().persist()
    },
    ...
    async addFolderToWorkspace(path) {
      ...
      if (!get().currentWorkspaceId) get().saveWorkspace(baseName(s.workspace))
    ...
    async openProjectConfig() {
      ...
      if (!(await window.lumen.fs.exists(file))) await saveProjectConfig(root, get().projectConfig)
    ...
    async setPreferredLsp(languageId, label) {
      ...
      const s = get()
      if (spec && s.effects.lsp) await lsp.applyPreference(spec, s.project?.root ?? s.workspace)
    },
