...
import type { AppearanceSlice, Slice } from '../types'
...
export const createAppearanceSlice: Slice<AppearanceSlice> = (set, get) => {
  ...
  function commitThemes(customThemes: Theme[]) {
    registry.setUserThemes(customThemes)
    set({ customThemes, registryVersion: registry.getVersion() })
  }

  function commitIconPacks(customIconPacks: IconPack[]) {
  ...
  async function writeJson(target: string | null, value: unknown, done: string) {
    if (!target) return
    try {
  ...
  return {
    ...
    openThemeStudio(baseId) {
      ...
      commitThemes([...s.customThemes, copy])
    ...
    deleteCustomTheme(id) {
      commitThemes(get().customThemes.filter((theme) => theme.id !== id))
    ...
    async exportTheme(id) {
      ...
      await writeJson(target, theme, t('notify.theme.exported', { name: theme.name }))
    },
    ...
    deleteCustomIconPack(id) {
      commitIconPacks(get().customIconPacks.filter((pack) => pack.id !== id))
    ...
    async exportIconPack(id) {
      ...
      await writeJson(target, pack, t('iconPacks.exported', { name: pack.name }))
    },
    ...
    toggleAddon(id) {
      ...
      if (!registry.themes().some((theme) => theme.id === get().themeId)) get().setTheme(DEFAULT_THEME_ID)
