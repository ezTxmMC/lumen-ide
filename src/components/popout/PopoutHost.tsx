...
export function PopoutHost() {
  ...
  useEffect(() => {
    for (const entry of popouts) {
      ...
      if (gone || !popoutWindow(entry.key)) {
        useStore.getState().dockBack(entry.key);
      }
    }
  ...
  useEffect(() => {
    const onFocus = () => {
      ...
      if (shown.some((group) => group.id === s.activeGroupId)) {
        return;
      }
      const first = shown[0];
      if (first) {
        s.focusGroup(s.groups.indexOf(first));
      }
    };
  ...
  useEffect(() => useStore.subscribe((state, previous) => {
    if (!state.popouts.length || !overlayOpen(state) || overlayOpen(previous)) {
      return;
    }
    window.focus();
  }), []);
