...
export function NavStrip({ side }: { side: 'left' | 'right' }) {
  ...
  if (!primary && !views.length && !dragging) return null
  ...
  const onDrop = (event: React.DragEvent) => {
    ...
    setDropAt(null)
    if (id) useStore.getState().moveView(id, side, index)
  }
    ...
    >
      ...
      {views.map((view, index) => (
        <StripButton
          ...
          register={(el) => { if (el) buttons.current.set(view.id, el) }}
          onContextMenu={(event) => {
            event.preventDefault()
      ...
      {primary && DIALOGS.map(({ id, icon: Icon, label, command }) => (
        <button
          key={id}
