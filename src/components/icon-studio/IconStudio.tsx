...
function starterDef(section: MapKey, key: string): IconDef {
  if (section === 'folderNames') return { shape: 'folder', color: '#7c8cff' }
  if (section === 'fileNames') return { shape: 'file', color: '#9aa3b0' }
...
function StudioWindow({ initial }: { initial: IconPack }) {
  ...
  const cancel = () => {
    if (dirty && !confirm(t('iconPacks.studio.discard'))) return
    close()
  }
    ...
    <div className="lm-anim-fade fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
      ...
      >
        <header className="flex shrink-0 flex-wrap items-center gap-2.5 border-b border-edge px-4 py-2.5">
          ...
          <Button size="sm" variant="outline" onClick={cancel}>{t('iconPacks.studio.cancel')}</Button>
          <Button size="sm" variant="solid" disabled={problems.length > 0} onClick={commit}>{t('iconPacks.studio.save')}</Button>
          <Button size="sm" title={t('common.closeEsc')} onClick={cancel}><X size={14} /></Button>
        </header>
