...
import { resolveValues, validateValues } from '@/core/project/scaffold'
...
export function FormDialog() {
  ...
  const values = useMemo(() => resolveValues(spec?.fields ?? [], {}, touched), [spec, touched])
  const errors = useMemo(() => validateValues(spec?.fields ?? [], values), [spec, values, language])
  ...
  const firstText = spec.fields.find((f) => (f.type ?? 'text') === 'text' && (!f.when || f.when(values)))
