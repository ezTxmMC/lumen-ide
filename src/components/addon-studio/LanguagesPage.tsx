...
export function newLanguage(existing: UserLanguage[]): UserLanguage {
  ...
  while (existing.some((l) => l.id === `sprache${n}`)) n++
  return {
    id: `sprache${n}`,
...
export function LanguagesPage({
  ...
  const regexError = (value: string | undefined) => checkRegex(value)
    ...
    <div className="flex h-full min-h-0">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto px-4 py-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          <Button size="sm" variant="danger" title={t('common.delete')} onClick={() => {
            if (!confirm(t('common.confirmDelete', { name: lang.name || lang.id }))) return
      ...
      <div className="flex w-[360px] shrink-0 flex-col gap-2 border-l border-edge p-3">
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" title={t('addonStudio.languages.editSample')} onClick={() => setEditSample((v) => !v)}>
            <Pencil size={11} />
          </Button>
        </div>
        {editSample && (
          <textarea
            ...
            rows={8}
            spellCheck={false}
            onChange={(e) => setSample({ ...sample, [lang.id]: e.target.value })}
