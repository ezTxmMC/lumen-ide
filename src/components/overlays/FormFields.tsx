...
function choiceKind(field: FormField, choices: FieldChoice[]): 'select' | 'combobox' | null {
  if (field.type === 'combobox') return 'combobox'
  if (field.type !== 'select') return null
  if (choices.length > COMBOBOX_THRESHOLD) return 'combobox'
  return 'select'
}
    ...
    <div className={kind ? '' : 'min-w-0'}>
      ...
      {field.type === 'textarea' && (
        <textarea
          ...
          className={`${inputClass} ${border} ${mono} resize-y`}
        />
      )}
      {kind === 'select' && (
        <select
      ...
      {shownError && <span className="mt-1 block text-[11px] text-bad">{shownError}</span>}
      {!shownError && !failed && field.hint && <span className="mt-1 block text-[11px] leading-snug text-subtle">{tr(field.hint)}</span>}
      {!shownError && !failed && current?.hint && kind === 'select' && (
  ...
  return (
    <>
      {groupChoices(choices).map((group) => {
        if (!group.group) return group.choices.map(option)
