...
function ComboboxPopover({
  ...
  const [place, setPlace] = useState<{ left: number; top: number; width: number; maxHeight: number; above: boolean } | null>(null)
      ...
      >
        <div className="flex items-center gap-2 border-b border-edge px-2.5 py-1.5">
          ...
          <input
            ...
            aria-activedescendant={order[active] ? `choice-${active}` : undefined}
            spellCheck={false}
          ...
          <span className="shrink-0 text-[10.5px] tabular-nums text-subtle">{matches.length}</span>
        </div>
