...
function OklchSliders({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  if (lch.c > 0.002) hue.current = lch.h
  const set = (patch: Partial<typeof lch>) => {
    const next = { ...lch, h: hue.current, ...patch }
    if (patch.h !== undefined) hue.current = patch.h
...
export function ColorField({
  ...
  const commitText = (next: string) => {
    ...
    setText(trimmed)
    if (isHexColor(trimmed)) onChange(toHex(parseHex(trimmed)!), `${colorKey}:text`)
  }
