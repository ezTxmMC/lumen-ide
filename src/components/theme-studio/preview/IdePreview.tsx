...
export function IdePreview({ theme }: { theme: Theme }) {
  ...
  const rowStyle = (state?: string) => {
    if (state === 'selected') return { background: ui.bgActive, color: ui.text }
    if (state === 'hover') return { background: ui.bgHover, color: ui.text }
    if (state === 'modified') return { color: ui.warning }
    if (state === 'error') return { color: ui.danger }
    return { color: ui.textMuted }
  }
  const rowKeys = (state?: string): ColorKey[] => {
    if (state === 'selected') return ['ui:bgActive', 'ui:text']
    if (state === 'hover') return ['ui:bgHover', 'ui:text']
    if (state === 'modified') return ['ui:warning']
    if (state === 'error') return ['ui:danger']
    return ['ui:textMuted']
  }
