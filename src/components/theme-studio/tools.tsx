...
function IssuesSection({ draft, change, labelOf, onSelect }: ToolProps & {
...
function MatrixSection({ draft, labelOf }: ToolProps) {
  const t = useT();
  return (
...
function SyntaxContrastSection({ draft, labelOf, onSelect }: ToolProps & { onSelect(key: ColorKey): void; }) {
  const t = useT();
  return (
...
export function ContrastTools({
  ...
  return (
    <>
      <IssuesSection draft={draft} change={change} labelOf={labelOf} onSelect={onSelect} />
      <MatrixSection draft={draft} labelOf={labelOf} />
      <SyntaxContrastSection draft={draft} labelOf={labelOf} onSelect={onSelect} />
    </>
  );
}
