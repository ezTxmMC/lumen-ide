...
function speedLabel(t: (key: string) => string, v: number): string {
  if (v < 0.9) {
    return t('themeStudio.effects.fast');
  }
  if (v > 1.4) {
    return t('themeStudio.effects.calm');
  }
...
function LevelPicker({ effects, setEffects, onHover }: EffectsProps & { onHover(level: Level | null): void; }) {
  const t = useT();
  return (
...
function MotionSection({ effects, setEffects }: EffectsProps) {
  const t = useT();
  ...
  return (
    <Section title={t('themeStudio.effects.motion')}>
      ...
      <LevelPicker effects={effects} setEffects={setEffects} onHover={setHoverLevel} />
...
function LooksSection({ effects, setEffects, show }: EffectsProps & { show(...labels: string[]): boolean; }) {
  const t = useT();
  return (
    <Section title={t('themeStudio.effects.looks')}>
      ...
      {/* Live sample for glass, glow, shadow and corners */}
...
export function EffectsSection({ query }: { query: string; }) {
  ...
  return (
    <div className="mx-auto max-w-[760px] px-4 py-3">
      ...
      {motionVisible && <MotionSection effects={effects} setEffects={setEffects} />}

      {looksVisible && <LooksSection effects={effects} setEffects={setEffects} show={show} />}
