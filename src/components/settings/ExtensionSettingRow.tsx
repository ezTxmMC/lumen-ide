...
export function settingChanged(values: Record<string, string> | undefined, setting: ExtensionSetting): boolean {
  if (setting.type === 'secret') return false
    ...
    <div className="py-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <input
          ...
          onChange={(e) => setDraft(e.target.value)}
          onKeyDown={(e) => { if (e.key === 'Enter' && draft) void save(draft) }}
          className={inputClass}
        />
...
function SettingControl({ extensionId, extensionName, setting: declared }: {
  ...
  if (setting.type === 'multiselect') return <MultiSelectSetting setting={setting} label={label} current={current} write={write} />
  if (setting.type === 'list') return <ListSetting setting={setting} label={label} current={current} write={write} />

  if (setting.type === 'secret') return <SecretSetting extensionId={extensionId} setting={setting} label={label} />
