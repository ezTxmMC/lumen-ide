...
function useExtensionInstall(servers: ExtensionServer[], activeServers: ExtensionServer[]) {
  const t = useT();
...
function entryMatches(entry: ExtensionSummary, needle: string): boolean {
  if (!needle) {
    return true;
  }
...
export function ExtensionsDialog() {
  ...
  const { section, setSection, installedFilter } = useDialogRouting(open, requested);
  ...
  const { busy, install, updateAll } = useExtensionInstall(servers, activeServers);
      ...
      {section === 'updates' && (
        <UpdateList
          ...
          onUpdate={(update) => {
            ...
            if (entry) {
              void install(update.server, entry.summary, update.to);
            }
          }}
...
function ExploreList({ entries, servers, category, onCategory, busy, providesText, onInstall }: {
  ...
  const body = () => {
    if (!servers.length) {
      return <Empty icon={<Globe size={22} />} title={t('extensions.noServers')} hint={t('extensions.subtitle')} />;
    }
    if (loading && !entries.length) {
      return <Empty icon={<RefreshCw size={22} className="lm-anim-spin" />} title={t('extensions.loading')} />;
    }
    if (!entries.length) {
      return <Empty icon={<Blocks size={22} />} title={t('extensions.noResults')} />;
    }
    return (
      <div className="flex flex-col gap-2">
        {entries.map(({ summary, server }) => {
          ...
          const label = () => {
            if (!fits) {
              return t('extensions.requiresApp', { required: summary.minAppVersion ?? '' });
            }
            if (outdated) {
...
function UpdateList({ updates, servers, busy, onUpdate, onUpdateAll }: {
  ...
  const body = () => {
    ...
    if (!updates.length) {
      return <Empty icon={<CheckCircle2 size={22} />} title={t('extensions.updatesNone')} />;
    }
    return (
      <div className="flex flex-col gap-2">
        {updates.map((update) => {
