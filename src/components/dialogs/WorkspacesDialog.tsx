...
export function WorkspacesDialog() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      for (const folder of workspaces.flatMap((w) => w.folders)) {
        if (!(await window.lumen.fs.exists(folder))) gone.add(folder)
      }
  ...
  const commitRename = (def: WorkspaceDef) => {
    ...
    setEditing(null)
    if (name && name !== def.name) store.updateWorkspace(def.id, { name })
  }
      ...
      headerExtra={
        <div className="flex items-center gap-1">
          <Button size="sm" variant="outline" disabled={!workspace} onClick={() => {
            ...
            if (def) store.notify(t('workspaces.saved', { name: def.name }), 'success')
          }}>
        ...
        {workspace && (
          <section className="mb-5">
            <h3 className="mb-1 text-[10.5px] font-semibold uppercase tracking-[0.09em] text-subtle">
              {currentId ? workspaces.find((w) => w.id === currentId)?.name : t('workspaces.folderOnly')}
            </h3>
            ...
            return (
              ...
              >
                ...
                <div className="mt-2.5 flex items-center gap-1">
                  ...
                  <Button size="sm" title={t('workspaces.rename')} onClick={() => startRename(def)}><Pencil size={11} /></Button>
                  ...
                  <Button size="sm" variant="danger" title={t('workspaces.delete')} onClick={() => {
                    if (confirm(t('workspaces.confirmDelete', { name: def.name }))) store.deleteWorkspace(def.id)
                  }}>
                    <Trash2 size={11} />
                  </Button>
                </div>
