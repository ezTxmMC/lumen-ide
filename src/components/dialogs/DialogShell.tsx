...
export function DialogShell({
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || studioOpen) return
      if ((event.target as HTMLElement | null)?.closest?.('[data-keybinding-recorder]')) return
    ...
    >
      ...
      >
        <header className="flex shrink-0 items-center gap-3 border-b border-edge px-4 py-2.5">
          ...
          {headerExtra}
          <Button size="sm" title={t('common.closeEsc')} onClick={close}>
            <X size={14} />
          </Button>
        </header>
