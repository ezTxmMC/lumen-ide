...
function planTitle(t: Translate, plan: InstallPlan): string {
  if (plan.kind === 'managed') return t('lsp.dialog.kindManaged')
  if (plan.kind === 'system') return t('lsp.dialog.kindSystem', { manager: managerLabel(plan.manager) })
  return t('lsp.dialog.kindCommand')
}

function planHint(t: Translate, plan: InstallPlan): string {
  if (plan.kind === 'managed') return t('lsp.dialog.kindManagedHint')
  if (plan.kind === 'system') return t('lsp.dialog.kindSystemHint')
...
function Chooser({ request, onClose }: { request: InstallRequest; onClose: (decline: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    void Promise.all(request.servers.map(async (config) => [config.label, await lsp.isInstalled(config)] as const))
      .then((entries) => { if (alive) setInstalled(Object.fromEntries(entries)) })
    return () => { alive = false }
  }, [request])
  ...
  const targets = useMemo<InstallTarget[]>(() => {
    ...
    return request.servers.flatMap((config) => {
      if (!choice.selected.includes(config.label)) return []
      const plans = lsp.installPlans(config)
      const plan = plans.find((entry) => planKey(entry) === choice.methods[config.label]) ?? plans[0]
      if (!plan) return []
  ...
  const toggle = (label: string) => setChoice((current) => {
    if (request.mode === 'language') return { ...current, selected: [label] }
...
function summaryOf(t: Translate, state: InstallState): { text: string; tone: string } {
  ...
  if (cancelled && !ok) return { text: t('lsp.dialog.doneCancelled'), tone: 'text-subtle' }
  if (ok === state.results.length && ok > 0) return { text: t('lsp.dialog.doneOk'), tone: 'text-ok' }
  if (ok > 0) return { text: t('lsp.dialog.donePartial'), tone: 'text-warn' }
  return { text: t('lsp.dialog.doneFailed'), tone: 'text-bad' }
}

function resultLabel(t: Translate, result: InstallState['results'][number]) {
  if (result.ok) return <span className="text-ok">{t('lsp.dialog.resultOk')}</span>
  if (result.cancelled) return <span className="text-subtle">{t('lsp.dialog.resultCancelled')}</span>
