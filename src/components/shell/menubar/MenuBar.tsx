...
export function MenuBar() {
  ...
  const activate = (level: number, index: number, element: HTMLElement | null) => {
    ...
    if (row.kind === 'submenu') {
      if (element) openSubmenu(level, index, element, true)
      return
    }
  ...
  const latest = useRef({ open, focused, stack, menus, openMenu, close, setActive, openSubmenu, activate })
  latest.current = { open, focused, stack, menus, openMenu, close, setActive, openSubmenu, activate }
  ...
  const engaged = open !== null || focused !== null
  ...
  useEffect(() => {
    ...
    const onPointer = (event: MouseEvent) => {
      ...
      if (target?.closest(`[${MENUBAR_ATTR}]`)) return
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      const handled = latest.current.open === null ? barKey(event) : menuKey(event)
      if (!handled) return
      event.preventDefault()
  ...
  function barKey(event: KeyboardEvent): boolean {
    const state = latest.current
    const count = state.menus.length
    const current = state.focused ?? 0
    const keys: Record<string, () => void> = {
      ArrowRight: () => setFocused((current + 1) % count),
      ArrowLeft: () => setFocused((current - 1 + count) % count),
  ...
  function menuKey(event: KeyboardEvent): boolean {
    ...
    if (!top || state.open === null) return false
    const count = state.menus.length
    ...
    const keys: Record<string, () => void> = {
      ...
      ArrowLeft: () => {
        if (level > 0) {
          setStack((current) => current.slice(0, level))
          return
        }
    ...
    jumpToLetter(level, top, event.key)
    return true
  }

  /** A letter: the next entry whose label starts with it. */
  function jumpToLetter(level: number, top: Level, key: string) {
    ...
    for (let offset = 1; offset <= count; offset++) {
      ...
      if (!isSelectable(row) || row.kind === 'sep') continue
      if (!row.label.toLowerCase().startsWith(letter)) continue
      latest.current.setActive(level, index)
      return
    }
  }
