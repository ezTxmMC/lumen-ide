...
function toRow(entry: MenuEntry, commands: CommandIndex): Row | null {
  if (entry === 'sep') return { kind: 'sep' }
  if ('command' in entry) return commandRow(entry, commands)
  if ('submenu' in entry) {
...
export function resolveRows(entries: MenuEntry[], commands: CommandIndex): Row[] {
  ...
  for (const entry of entries) {
    ...
    if (!row) continue
    if (row.kind === 'sep' && (!rows.length || rows[rows.length - 1].kind === 'sep')) continue
    rows.push(row)
  }
  if (rows[rows.length - 1]?.kind === 'sep') rows.pop()
  return rows
}
