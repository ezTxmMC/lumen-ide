...
export function StatusBar() {
  ...
  return (
    <footer className="flex h-6 shrink-0 items-center gap-3 border-t border-edge bg-surface px-3 text-[11px] text-subtle">
      {workspace && <ProjectChip workspace={workspace} project={project} onClick={() => showSidebar('project')} />}
      ...
      <ProblemCounts errors={counts.errors} warnings={counts.warnings} title={t('statusbar.problemsWorkspace')} onClick={() => showPanel('problems')} />
      ...
      {tab && <ProblemCounts errors={fileErrorCount} warnings={fileWarnCount} title={t('statusbar.problemsFile')} onClick={() => showPanel('problems')} />}

      {tab && !tab.viewer && !parseViewTabPath(tab.path) && <EditorInfo tab={tab} lines={lines} cursor={cursor} language={language} onSymbols={() => setPalette('symbols')} onLanguage={() => setPalette('commands')} />}
...
function SymbolBreadcrumb({ path, cursor }: { path: string | null; cursor: StoreState['cursor']; }) {
  const t = useT();
...
function ProjectChip({ workspace, project, onClick }: { workspace: string; project: StoreState['project']; onClick(): void; }) {
  return (
...
function ProblemCounts({ errors, warnings, title, onClick }: { errors: number; warnings: number; title: string; onClick(): void; }) {
  if (errors === 0 && warnings === 0) {
    return null;
  }
  return (
    <button onClick={onClick} className="lm-transition flex items-center gap-2 rounded px-1 hover:bg-hover" title={title}>
