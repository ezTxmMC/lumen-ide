...
function shellMenuItems(t: Translate, shells: Shells, openTerminal: (options: { shell: string; }) => Promise<unknown>): MenuItem[] {
  const header: MenuItem = { header: t('panels.terminal.newWith') };
  if (!shells.length) {
    return [header, { label: t('panels.terminal.noShell'), disabled: true, run: () => {} }];
  }
...
function externalMenuItems(t: Translate, externals: Externals, openExternalTerminal: (cwd: undefined, id: string) => Promise<unknown>): MenuItem[] {
  const header: MenuItem = { header: t('panels.terminal.external') };
  if (!externals.length) {
    return [header, { label: t('panels.terminal.noExternal'), disabled: true, run: () => {} }];
  }
    ...
    >
      ...
      {renaming ? (
        <input
          ...
          onBlur={(e) => { terminals.rename(session.id, e.target.value); onRename(null); }}
          onKeyDown={(e) => {
            e.stopPropagation();
            if (e.key === 'Enter') {
        ...
        <span className="max-w-[140px] truncate">{index + 1}: {session.title}</span>
      )}
      <button
  ...
  return (
    <div className="flex min-w-0 items-center gap-0.5">
      ...
      <div className="flex shrink-0 items-center">
        ...
        {menu && <ContextMenu x={menu.x} y={menu.y} items={menu.kind === 'shells' ? shellMenuItems(t, shells, openTerminal) : externalMenuItems(t, externals, openExternalTerminal)} onClose={() => setMenu(null)} />}
      </div>
    </div>
  );
}
...
export function TerminalPanel() {
  ...
  useEffect(() => {
    if (started.current || terminals.list().length > 0 || terminals.pending > 0) {
      return;
    }
