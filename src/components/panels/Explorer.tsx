...
import {
  ChevronRight, ChevronsDownUp, Clipboard, ExternalLink, FilePlus, FolderInput, FolderOpen, FolderPlus,
  PenLine, RefreshCw, SquareTerminal, Star, Trash2, X,
} from 'lucide-react'
...
function rowTone(selected: boolean, active: boolean): string {
  if (selected) return 'bg-active text-fg'
...
function TreeNode({ entry, depth, api, label = entry.name, packages = false }: {
  ...
  const isSelected = api.selected?.path === entry.path
  useIconPackVersion()
...
function ContextMenu({ menu, api, onClose }: { menu: MenuState; api: TreeApi; onClose: () => void }) {
  ...
  const copy = async (text: string) => {
    try {
      ...
      notify(t('explorer.copied'), 'info')
    } catch {
      notify(t('explorer.clipboardUnavailable'), 'warning')
    }
  }
  ...
  if (entry) {
    items.push(
      ...
      'sep',
      { label: t('shell.groups.copyPath'), icon: Clipboard, run: () => void copy(entry.path) },
      { label: t('explorer.copyRelativePath'), icon: Clipboard, run: () => void copy(relativeToWorkspace(entry.path, workspace)) },
...
export function Explorer() {
  ...
  const pathDeleted = useStore((s) => s.pathDeleted)
  ...
  useEffect(() => {
    ...
    const offFs = window.lumen.fs.onChanged(schedule)
  ...
  const api: TreeApi = useMemo(() => ({
    ...
    async commitCreate(name) {
      ...
      setSelected({ name: clean.split(/[\\/]/).pop() ?? clean, path: target, isDirectory: isDir })
      refresh()
      if (isDir) return
    ...
    async commitRename(entry, name) {
      ...
      setSelected({ ...entry, name, path: target })
      refresh()
    },
    async remove(entry) {
      const question = entry.isDirectory ? 'explorer.confirmTrashFolder' : 'explorer.confirmTrashFile'
      if (!confirm(t(question, { name: entry.name }))) return
      try {
        await window.lumen.fs.remove(entry.path)
      } catch (err) {
        notify((err as Error).message.replace(/^Error: /, ''), 'error')
        return
      }
      pathDeleted(entry.path)
  ...
  }), [root, expanded, selected, renaming, creating, refreshToken, notify, openFile, pathRenamed, pathDeleted, refresh])
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      >
        ...
        <div className="h-8" onClick={() => setSelected(null)} onContextMenu={(e) => api.openMenu(e, null)} />
      </div>
