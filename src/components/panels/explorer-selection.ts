...
export function clickSelection(
  ...
  if (modifiers.range) {
    ...
    if (!range) return single(path)
    if (!modifiers.toggle) return { paths: range, anchor: current.anchor ?? path }
    const merged = [...current.paths, ...range.filter((p) => !current.paths.includes(p))]
    return { paths: merged, anchor: current.anchor ?? path }
  }
  if (modifiers.toggle) {
    if (current.paths.includes(path)) return { paths: current.paths.filter((p) => p !== path), anchor: path }
