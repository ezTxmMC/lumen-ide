...
export function cutPaths(paths: string[]) {
  if (paths.length) {
    setClipboard({ mode: 'cut', paths: topLevel(paths) });
  }
}

export function copyPaths(paths: string[]) {
  if (paths.length) {
    setClipboard({ mode: 'copy', paths: topLevel(paths) });
  }
}
...
export async function movePaths(paths: string[], target: string): Promise<string[]> {
  ...
  for (const path of plan.invalid) {
    state.notify(t('explorer.moveIntoItself', { name: baseName(path) }), 'warning');
  }
...
export async function pasteInto(target: string): Promise<string[]> {
  ...
  if (job.mode === 'copy') {
    return copyPathsInto(job.paths, target);
  }
