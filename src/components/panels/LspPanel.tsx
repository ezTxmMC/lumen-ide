            ...
            return (
              <div
                ...
                className={[
                  ...
                  filter === server.id ? 'border-accent bg-active' : 'border-edge hover:bg-hover',
                ].join(' ')}
                onClick={() => setFilter(filter === server.id ? null : server.id)}
              ...
              >
                ...
                <div className="mt-0.5 flex items-center gap-1 text-[10.5px] text-subtle">
                  ...
                  <span className="flex shrink-0 items-center gap-0.5" onClick={(e) => e.stopPropagation()}>
                    <button title={t('panels.lsp.restart')} onClick={() => void restart(server.id)} className="lm-transition rounded p-0.5 hover:text-fg"><RotateCw size={11} /></button>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-2 border-b border-edge px-2 py-1 text-[11px] text-subtle">
          ...
          <label className="flex items-center gap-1">
            <input type="checkbox" checked={stderr} onChange={(e) => setStderr(e.target.checked)} /> stderr
          </label>
