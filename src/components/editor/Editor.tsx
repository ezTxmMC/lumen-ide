...
function stateForTab(groupId: string, tabId: string): EditorState | undefined {
  ...
  for (const key of editorStates.keys()) {
    if (key === own || !key.endsWith(`:${tabId}`)) continue
    ...
    if (stillShown) continue
    if ((stateStamps.get(key) ?? 0) > (stateStamps.get(best) ?? 0)) best = key
  }
...
function middleChange(doc: string, next: string) {
  ...
  while (start < max && doc.charCodeAt(start) === next.charCodeAt(start)) start++
...
function forgetTab(tabId: string) {
  ...
  for (const key of [...editorStates.keys()]) {
    if (!key.endsWith(`:${tabId}`)) continue
...
export function Editor({ groupId }: { groupId: string }) {
  ...
  useEffect(() => {
    ...
    if (!tab || tab.viewer) {
      current.current = null
      if (tab && editorBridge.view === cm) editorBridge.attach(null, null)
      return
    }
  ...
  useEffect(() => {
    ...
    if (!cm.hasFocus && !cm.dom.ownerDocument.activeElement?.closest('input, textarea, .lm-terminal, [role="dialog"]')) cm.focus()
    setCursor(offsetToPos(cm.state.doc, cm.state.selection.main.head))
