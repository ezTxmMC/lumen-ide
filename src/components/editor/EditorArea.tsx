...
/** The context menu of a tab. */
function tabMenuItems(tabId: string, { t, group, popped, closeTab, pinTab }: TabMenuEnv): MenuItem[] {
  const s = useStore.getState();
  ...
  return [
    ...
    { label: t('shell.groups.closeOthers'), run: () => { s.setActiveTab(tabId, group.id); s.closeOthers(tabId); }, disabled: group.tabIds.length < 2 },
    {
      ...
      run: () => { for (const id of group.tabIds.slice(index + 1)) {
        useStore.getState().closeTab(id, group.id);
      } },
    },
    { label: t('shell.groups.closeAll'), run: () => { for (const id of group.tabIds) {
      useStore.getState().closeTab(id, group.id);
    } } },
    'sep',
    ...
    },
    {
      ...
      run: () => { if (tab?.path) {
        void window.lumen.shell.showItemInFolder(tab.path);
      } },
    },
  ];
}

function TabItem({ tab, index, isLast, groupId, active, groupActive, multiple, dropIndex, onDropIndex, onDrop, onMenu }: {
  tab: Tab;
  index: number;
  isLast: boolean;
  groupId: string;
  ...
  return (
    <div
      ...
      onDragOver={(e) => {
        ...
        onDropIndex(e.clientX < rect.left + rect.width / 2 ? index : index + 1);
      }}
      onDrop={(e) => onDrop(e, dropIndex ?? index)}
      onClick={() => setActiveTab(tab.id, groupId)}
      onDoubleClick={() => pinTab(tab.id)}
      onAuxClick={(e) => e.button === 1 && closeTab(tab.id, groupId)}
    ...
    >
      ...
      {dropIndex === index + 1 && isLast && <span className="lm-anim-fade absolute inset-y-1 right-0 w-[2px] rounded-full bg-accent" />}
...
function EditorTabs({ group, popped }: { group: EditorGroup; popped: boolean; }) {
  ...
  return (
    ...
    >
      ...
      <GroupActions group={group} multiple={multiple} popped={popped} />

      {menu && <ContextMenu x={menu.x} y={menu.y} items={tabMenuItems(menu.tabId, { t, group, popped, closeTab, pinTab })} onClose={() => setMenu(null)} />}
    </div>
  );
}
...
function SvgModeToggle({ tabId }: { tabId: string | null; }) {
  ...
  if (!tab?.path || tab.virtual || !isSvgPath(tab.path)) {
    return null;
  }
