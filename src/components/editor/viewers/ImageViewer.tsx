...
/** Fit-or-manual view of the image: zoom, pan and the wheel, all against the latest view. */
function useImageView(viewport: React.RefObject<HTMLDivElement | null>, image: Size, size: Size, failed: boolean) {
  ...
  return { view, fit, setFit, zoomTo, actualSize, startPan };
}

/** Zoom, fit and 100 % buttons of the info bar. */
function ZoomTools({ view, fit, onZoom, onFit, onActual }: {
  view: View;
  fit: boolean;
  ...
  return (
    <>
      <Button size="sm" title={t('media.zoomOut')} onClick={() => onZoom(view.scale / STEP)}><Minus size={12} /></Button>
      <span className="w-12 text-center text-[11px] tabular-nums text-muted" data-testid="media-zoom">{percent}</span>
      <Button size="sm" title={t('media.zoomIn')} onClick={() => onZoom(view.scale * STEP)}><Plus size={12} /></Button>
      <Button size="sm" title={t('media.fit')} onClick={onFit} className={fit ? 'text-accent' : ''}>
        <Maximize size={12} /> {t('media.fitShort')}
      </Button>
      <Button size="sm" title={t('media.actualSize')} onClick={onActual}>
        <Scan size={12} /> 100 %
      </Button>
    </>
  );
...
export function ImageViewer({ tab }: { tab: Tab; }) {
  ...
  const { view, fit, setFit, zoomTo, actualSize, startPan } = useImageView(viewport, image, size, failed);

  const tools = <ZoomTools view={view} fit={fit} onZoom={zoomTo} onFit={() => setFit(true)} onActual={actualSize} />;
