...
class Minimap {
  ...
  /** Sizes the canvas to the minimap and returns a cleared 2D context, or null when there is nothing to draw on. */
  private prepareCanvas(width: number, height: number): CanvasRenderingContext2D | null {
    const { canvas } = this;
  ...
  /** Token colours per range of the drawn lines. */
  private collectSpans(from: number, to: number, plain: string) {
  ...
  /** The words of every drawn line as runs of blocks in their token's colour. */
  private drawText(
    ctx: CanvasRenderingContext2D,
    frame: { offset: number; startLine: number; endLine: number; },
    ...
    const { offset, startLine, endLine } = frame;
    let spanIndex = 0;
    for (let n = startLine; n <= endLine; n++) {
      ...
      for (let i = 0; i < line.length && column < MAX_COLUMNS; i++) {
        ...
        while (spanIndex < spans.length && spans[spanIndex].to <= pos) {
          spanIndex++;
        }
        const span = spans[spanIndex];
        const color = span && span.from <= pos ? span.color : plain;
        if (runStart >= 0 && (color !== runColor || options.characters)) {
          flush(column);
        }
        if (runStart < 0) {
          runStart = column;
  ...
  private drawDiagnostics(ctx: CanvasRenderingContext2D, width: number, height: number, total: number) {
  ...
  private draw() {
    ...
    const spans = this.collectSpans(doc.line(startLine).from, doc.line(endLine).to, plain);
    this.drawText(ctx, { offset, startLine, endLine }, spans, plain);
    this.drawDiagnostics(ctx, width, height, total);
...
export function minimap(options: MinimapOptions): Extension {
  return [
    ...
    // Through the facet rather than `classList`: CodeMirror rewrites the editor's own class
    // whenever its attributes change (focus, say) and would drop a class added by hand — the
    // scroller then lost its margin and code ran on behind the minimap.
    EditorView.editorAttributes.of({ class: 'lm-has-minimap', style: `--minimap-width: ${options.width}px` }),
