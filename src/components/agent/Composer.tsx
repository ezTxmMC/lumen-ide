...
export function Composer({ info, running }: { info: AgentInfo; running: boolean }) {
  ...
  const suggestions = useMemo(() => {
    if (!token) return []
    if (token.trigger === '/') return rank(slashCommands, token.query)
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (suggestions.length && ['ArrowDown', 'ArrowUp', 'Enter', 'Tab', 'Escape'].includes(event.key)) {
      ...
      if (event.key === 'ArrowDown') setSelected((i) => (i + 1) % suggestions.length)
      if (event.key === 'ArrowUp') setSelected((i) => (i - 1 + suggestions.length) % suggestions.length)
      if (event.key === 'Enter' || event.key === 'Tab') accept(suggestions[selected])
        ...
        <div role="listbox" className="lm-glass lm-shadow absolute right-2 bottom-full left-2 mb-1 max-h-56 overflow-y-auto rounded-lumen border border-edge p-1">
          ...
          {suggestions.map((value, i) => (
            <button
              ...
              onMouseDown={(e) => { e.preventDefault(); accept(value) }}
      ...
      <div className="mt-1.5 flex items-center gap-1.5">
        ...
        {info.agent.images && (
          <>
            ...
            />
            <Button size="sm" title={t('agent.attach')} onClick={() => picker.current?.click()} disabled={!workspace}>
              <ImagePlus size={12} />
            </Button>
          </>
        )}
