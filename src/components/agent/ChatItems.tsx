...
function fileOf(input: Record<string, unknown>): string | null {
  ...
  if (typeof file !== 'string' || !file.startsWith('/')) return null
  return file
}
...
function ResultFooter({ item }: { item: ItemOf<'result'> }) {
  ...
  if (!parts.length && !item.isError) return null
  return (
...
export function ChatMessage({ agentKey, item }: { agentKey: string; item: ChatItem }) {
  ...
  if (item.role === 'assistant') return <Markdown text={item.text} streaming={item.streaming} />
  if (item.role === 'thinking') return <ThinkingBlock item={item} />
  if (item.role === 'tool') return <ToolRow item={item} />
  if (item.role === 'permission') return <PermissionCard agentKey={agentKey} item={item} />
  if (item.role === 'todos') return <TodoCard item={item} />
  if (item.role === 'result') return <ResultFooter item={item} />
