...
function placeX(win: Window, width: number, x: number): number {
  ...
  if (x + width <= viewWidth - EDGE_MARGIN) return x
...
function placeY(win: Window, height: number, y: number): number {
  ...
  if (y + height <= viewHeight - EDGE_MARGIN) return y
...
function nextEnabled(items: MenuItem[], from: number, step: 1 | -1): number {
  for (let offset = 1; offset <= items.length; offset++) {
    ...
    if (isAction(item) && !item.disabled) return index
  }
  return -1
}
...
function rowKey(item: MenuItem, index: number): string {
  if (item === 'sep') return `sep-${index}`
  if (!isAction(item)) return `header-${index}`
...
function MenuRow({ item, focused, onHover, onRun }: {
  ...
  if (item === 'sep') return <div className="my-1 h-px bg-edge" />
  if (!isAction(item)) {
    return (
...
function RowIcon({ icon: Icon, checked }: { icon?: LucideIcon; checked?: boolean }) {
  if (checked) return <span className="w-3 shrink-0 text-center text-[11px] text-accent">✓</span>
  if (!Icon) return <span className="w-3 shrink-0" />
