...
export async function gotoLocation(
  ...
  const head = view.state.selection.main.head
  const position = offsetToPos(view.state.doc, head)
  const locations = await client[kind](filePath, position)
  const word = wordAt(view, head)
  if (!locations.length) {
    useStore.getState().notify(`Keine ${KIND_TITLE[kind]} gefunden${word ? ` für „${word}“` : ''}`, 'info')
    return false
  }
  ...
  await showLocations(`${KIND_TITLE[kind]}en von „${word}“`, locations)
  return true
}
...
export async function findReferences(view: EditorView, filePath: string): Promise<boolean> {
  ...
  const word = wordAt(view, head)
  if (!locations.length) {
    useStore.getState().notify(`Keine Referenzen${word ? ` für „${word}“` : ''}`, 'info')
    return false
  }
  await showLocations(`Referenzen von „${word}“ (${locations.length})`, locations)
  return true
}
