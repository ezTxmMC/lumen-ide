 ...
 * Detection
 * ---------
 * `detectProject` looks at file names in the project root alone. Minecraft
 * projects have no files of their own there — only `build.gradle(.kts)` or
 * `pom.xml`, as any other JVM project does. So that ordinary Gradle and Maven
 * projects are not taken for Minecraft, every kind brings a finer check
 * `detect` along, which reads the build files and the manifests (the Loom
 * plugin, paper-api, fabric.mod.json …).
 *
 * As long as the core does not read `detect`, the markers stay empty
 * (`CORE_SUPPORTS_KIND_DETECT = false`) — the kinds then do not appear at all
 * rather than wrongly. After the integration (see the report), turn the switch
 * over: the markers are then the build files and `detect` filters.
 */

import type { FormValues, ProjectContext, ProjectKind, ProjectMeta, ProjectTask } from '@/core/types'
import { grepValue, wrapperOr } from '@/core/project/detect'
import { t } from '@/i18n'

/** Turn over as soon as `detectProject` reads `kind.detect`. */
export const CORE_SUPPORTS_KIND_DETECT = true
...
const markers = (list: string[]) => (CORE_SUPPORTS_KIND_DETECT ? list : [])

/**
 * The project kind a template is detected as: the Minecraft kind once the core
 * reads `detect`, until then the build system.
 */
export function templateKind(id: string, values: FormValues): string {
  if (CORE_SUPPORTS_KIND_DETECT) return id
  return values.build === 'maven' ? 'maven' : 'gradle'
}
...
function listJars(ctx: ProjectContext, id: string, dir: string): ProjectTask {
  if (ctx.platform === 'win32') {
    return { id, label: t('minecraft.task.jars'), command: 'cmd', args: ['/c', 'dir', '/b', dir.replace(/\//g, '\\')], group: 'other', detail: dir }
  }
  return { id, label: t('minecraft.task.jars'), command: 'ls', args: ['-1', dir], group: 'other', detail: dir }
}
...
const BUKKIT_API = /(io\.papermc\.paper|org\.spigotmc|org\.bukkit|cn\.dreeam\.leaf|org\.purpurmc\.purpur)[:"'\s<>/a-z-]*?(paper-api|spigot-api|bukkit|leaf-api|purpur-api)/
...
export const bukkitKind: MinecraftKind = {
  ...
  icon: 'MC',
  color: '#62b47a',
  markers: markers([...GRADLE_MARKERS, ...MAVEN_MARKERS]),
  priority: 30,
  ...
  async tasks(ctx) {
    ...
    if (info.maven) {
      return [
        await mavenTask(ctx, 'mc:package', t('minecraft.task.build'), ['-q', 'package'], 'build'),
        await mavenTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['-q', 'clean'], 'clean'),
        listJars(ctx, 'mc:jars', 'target'),
      ]
    }
    const tasks = [
      await gradleTask(ctx, 'mc:build', t('minecraft.task.build'), ['build'], 'build'),
      await gradleTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['clean'], 'clean'),
    ]
    if (/run-paper/.test(info.build)) tasks.push(await gradleTask(ctx, 'mc:runServer', t('minecraft.task.runServer'), ['runServer'], 'run'))
  ...
  async inspect(ctx) {
    const info = await readBuild(ctx)
    const manifest = await firstFile(ctx, BUKKIT_MANIFESTS)
    const facts: Record<string, string> = { [t('minecraft.fact.platform')]: bukkitPlatform(info.build) }
...
export const velocityKind: MinecraftKind = {
  ...
  async tasks(ctx) {
    ...
    const tasks = [await gradleTask(ctx, 'mc:build', t('minecraft.task.build'), ['build'], 'build')]
    if (/run-velocity/.test(info.build)) tasks.push(await gradleTask(ctx, 'mc:runVelocity', t('minecraft.task.runProxy'), ['runVelocity'], 'run'))
    tasks.push(listJars(ctx, 'mc:jars', 'build/libs'))
    return tasks
  },
  async inspect(ctx) {
    const info = await readBuild(ctx)
    const facts: Record<string, string> = { [t('minecraft.fact.platform')]: 'Velocity' }
...
export const bungeeKind: MinecraftKind = {
  ...
  async tasks(ctx) {
    ...
    const tasks = [await gradleTask(ctx, 'mc:build', t('minecraft.task.build'), ['build'], 'build')]
    if (/run-waterfall/.test(info.build)) tasks.push(await gradleTask(ctx, 'mc:runWaterfall', t('minecraft.task.runProxy'), ['runWaterfall'], 'run'))
    tasks.push(listJars(ctx, 'mc:jars', 'build/libs'))
    return tasks
  },
  async inspect(ctx) {
    ...
    const facts: Record<string, string> = { [t('minecraft.fact.platform')]: /waterfall-api/.test(info.build) ? 'Waterfall' : 'BungeeCord' }
    const name = yamlValue(manifest?.text ?? '', 'name')
    if (name) facts.Plugin = name
    facts.Jar = info.maven ? 'target/*.jar' : 'build/libs/*.jar'
    return meta(info, facts, ['src/main/java', 'src/main/resources'], manifest?.path)
...
async function loomTasks(ctx: ProjectContext, info: BuildInfo): Promise<ProjectTask[]> {
  const tasks = [
    await gradleTask(ctx, 'mc:build', t('minecraft.task.build'), ['build'], 'build'),
    await gradleTask(ctx, 'mc:runClient', t('minecraft.task.runClient'), ['runClient'], 'run'),
    await gradleTask(ctx, 'mc:runServer', t('minecraft.task.runServer'), ['runServer'], 'run'),
  ]
  if (/configureDataGeneration|fabric-datagen/.test(info.build)) {
    tasks.push(await gradleTask(ctx, 'mc:runDatagen', t('minecraft.task.runData'), ['runDatagen'], 'other'))
  }
  ...
  if (!/net\.fabricmc\.fabric-loom['"]/.test(info.build)) {
    tasks.push(await gradleTask(ctx, 'mc:genSources', t('minecraft.task.genSources'), ['genSources'], 'other'))
  }
  tasks.push(
    await gradleTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['clean'], 'clean'),
...
export const fabricKind: MinecraftKind = {
  ...
  icon: 'MC',
  color: '#dbd0b4',
  markers: markers(GRADLE_MARKERS),
  priority: 32,
  languageIds: ['java', 'kotlin'],
  async detect(ctx) {
    const info = await readBuild(ctx)
...
export const neoforgeKind: MinecraftKind = {
  ...
  async tasks(ctx) {
    ...
    if (/register\(\s*["']data["']|^\s*data\s*\{/m.test(info.build)) tasks.push(await gradleTask(ctx, 'mc:runData', t('minecraft.task.runData'), ['runData'], 'other'))
    tasks.push(
      await gradleTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['clean'], 'clean'),
      listJars(ctx, 'mc:jars', 'build/libs'),
    )
...
export const forgeKind: MinecraftKind = {
  ...
  async detect(ctx) {
    const info = await readBuild(ctx)
    if (isArchitectury(info)) return false
    return /net\.minecraftforge\.gradle/.test(info.build)
  },
  async tasks(ctx) {
    const info = await readBuild(ctx)
    const tasks = [
    ...
    if (/register\(\s*['"]data['"]/.test(info.build)) tasks.push(await gradleTask(ctx, 'mc:runData', t('minecraft.task.runData'), ['runData'], 'other'))
    tasks.push(
      await gradleTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['clean'], 'clean'),
      listJars(ctx, 'mc:jars', 'build/libs'),
    )
...
export const architecturyKind: MinecraftKind = {
  ...
  async tasks(ctx) {
    ...
    for (const platform of platforms) {
      tasks.push(
        await gradleTask(ctx, `mc:${platform}:runClient`, `${t('minecraft.task.runClient')} (${platform})`, [`:${platform}:runClient`], 'run'),
        await gradleTask(ctx, `mc:${platform}:runServer`, `${t('minecraft.task.runServer')} (${platform})`, [`:${platform}:runServer`], 'run'),
      )
    }
    tasks.push(await gradleTask(ctx, 'mc:clean', t('minecraft.task.clean'), ['clean'], 'clean'))
...
function meta(info: BuildInfo, facts: Record<string, string>, sourceRoots: string[], manifest?: string): ProjectMeta {
  ...
  if (manifest) facts[t('minecraft.fact.manifest')] = manifest
  return {
