...
export function mavenModuleTasks(path: string, pom: PomSummary, pomText: string, mvn: string): ProjectTask[] {
  ...
  if (runner && pom.packaging !== 'pom') {
    tasks.splice(3, 0, task('run', 'templates.tasks.run', [...pl, runner[1]], 'run'));
  }
...
async function mavenChildren(
  ...
  for (const name of names) {
    ...
    if (!path || seen.has(path) || seen.size > MAX_MODULES) {
      continue;
    }
    seen.add(path);
...
export function parseGradleSettings(source: string): GradleSettings {
  ...
  for (const match of text.matchAll(override)) {
    dirs[gradlePath(match[1])] = joinRelative('', match[2]);
  }
...
export function parseGradlePlugins(source: string): string[] {
  ...
  for (const m of plugins.matchAll(/\bid\s*\(?\s*["']([^"']+)["']/g)) {
    found.add(m[1]);
  }
  for (const m of plugins.matchAll(/\bkotlin\s*\(\s*["']([^"']+)["']\s*\)/g)) {
    found.add(`org.jetbrains.kotlin.${m[1]}`);
  }
  for (const m of plugins.matchAll(/`([\w-]+)`/g)) {
    found.add(m[1]);
  }
  for (const m of plugins.matchAll(/\balias\s*\(\s*libs\.plugins\.([\w.]+)\s*\)/g)) {
    found.add(m[1].replace(/\./g, '-'));
  }
  for (const m of plugins.matchAll(/^[ \t]*([a-z][\w-]*)[ \t]*$/gm)) {
    found.add(m[1]);
  }
...
export function gradleModuleTasks(
  ...
  if (run) {
    tasks.splice(2, 0, task(run, 'templates.tasks.run', 'run'));
  }
...
export function gradleDependencies(script: string): ProjectDependency[] {
  return Array.from(stripComments(script).matchAll(GRADLE_DEPENDENCY)).map((m) => {
    if (m[3]) {
      return { name: gradlePath(m[3]), scope: m[1] };
    }
...
export async function gradleModuleTree(ctx: ProjectContext, gradle: string): Promise<ProjectModule[]> {
  ...
  for (const path of paths) {
    modules.set(path, await gradleModule(ctx, gradle, path, parsed.dirs, inherited));
  }
...
async function gradleModule(
  ...
  if (script === null) {
    return { id: path, name, path: dir, kind: 'container', tasks: [] };
  }
