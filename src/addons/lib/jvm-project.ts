...
const mavenDependencies: DependencySupport = {
  ...
  async add(ctx, dep) {
    if (!MAVEN_COORDINATE.test(dep.name)) {
      throw new Error('Erwartet groupId:artifactId');
    }
    const pom = await ctx.readFile('pom.xml');
    if (pom === null) {
      throw new Error('pom.xml fehlt');
    }
    ...
    if (at !== -1) {
      ...
      if (inserted) {
        return { type: 'edit', file: 'pom.xml', content: head + inserted };
      }
    }
    ...
    if (!anchor) {
      throw new Error(t('addons.pomNoProject'));
    }
...
export const mavenKind: ProjectKind = {
  ...
  async inspect(ctx) {
    ...
    if (!root) {
      return { buildFile: 'pom.xml', sourceRoots: ['src/main/java', 'src/test/java'] };
    }
...
const gradleDependencies: DependencySupport = {
  ...
  async add(ctx, dep) {
    ...
    if (text === null) {
      throw new Error('build.gradle(.kts) fehlt');
    }
    ...
    if (inserted) {
      return { type: 'edit', file, content: inserted };
    }
...
export const gradleKind: ProjectKind = {
  ...
  async inspect(ctx) {
    ...
    if (kotlinPlugin) {
      facts.Kotlin = kotlinPlugin[1] ?? t('templates.facts.yes');
    }
    ...
    if (toolchain) {
      facts.Java = toolchain[1] ?? toolchain[2];
    }
    const mainClass = /mainClass\s*(?:=|\.set\()\s*["']([^"']+)["']/.exec(build);
    if (mainClass) {
      facts['templates.fields.mainClass'] = mainClass[1];
    }
    if (await ctx.exists('gradle/libs.versions.toml')) {
      facts['templates.facts.versionCatalog'] = 'gradle/libs.versions.toml';
    }
    const includes = Array.from(settings.matchAll(/include\s*\(?\s*["']([^"']+)["']/g)).map((m) => m[1]);
    if (includes.length) {
      facts['templates.facts.modules'] = includes.join(', ');
    }
...
/** The `<dependency>` entries of a generated pom. */
function pomDependencies(tests: boolean, kotlin: boolean): string[] {
...
/** The `<plugin>` entries of a generated pom. */
function pomPlugins(java: string, kotlin: boolean, app: boolean, mainClass: string): string[] {
...
function pom(values: FormValues, opts: { kotlin: boolean; }): string {
  ...
  const dependencies = pomDependencies(tests, opts.kotlin);
  const plugins = pomPlugins(java, opts.kotlin, app, mainClass);
...
function gradleBuild(values: FormValues, opts: { kotlin: boolean; }): string {
  ...
  const plugins: string[] = [];
  if (opts.kotlin) {
    plugins.push(kts ? `kotlin("jvm") version "${values.kotlin || KOTLIN}"` : `id 'org.jetbrains.kotlin.jvm' version '${values.kotlin || KOTLIN}'`);
  }
  ...
  if (tests && opts.kotlin) {
    deps.push(kts ? 'testImplementation(kotlin("test"))' : "testImplementation 'org.jetbrains.kotlin:kotlin-test'");
  }
  if (tests && !opts.kotlin) {
    deps.push(
...
function buildCommands(values: FormValues) {
  ...
  if (values.build === 'maven') {
    return [
      ...(app ? [`mvn compile exec:java   # ${t('addons.mvnRun')}`] : []),
      `mvn test                # ${t('addons.mvnTest')}`,
      `mvn package             # ${t('addons.mvnPackage')}`,
    ];
  }
  return [
    ...(app ? [`gradle run     # ${t('addons.mvnRun')}`] : []),
    `gradle test    # ${t('addons.mvnTest')}`,
    `gradle build   # ${t('addons.gradleBuild')}`,
  ];
}
