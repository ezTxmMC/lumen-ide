...
export default function App() {
  ...
  const [withoutProject, setWithoutProject] = useState(startsEmpty);
  ...
  const [openingProject, setOpeningProject] = useState(() => new URLSearchParams(window.location.search).has('project') || new URLSearchParams(window.location.search).has('workspace'));
  // In the project screen's window “continue without a project” means the main window, without one.
  const continueWithoutProject = () => (isProjectsWindow ? void window.lumen.window.handOver({ empty: true }) : setWithoutProject(true));
  const showProjects = isProjectsWindow || (!workspace && !hasTabs && !withoutProject && !openingProject);
  ...
  useEffect(() => {
    ...
    const off = useStore.subscribe((state, previous) => {
      if (state.tabs === previous.tabs || timer) {
        return;
      }
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
      ...
      {showProjects ? <ProjectScreen onContinue={continueWithoutProject} /> : <Workbench />}

      {!isProjectsWindow && <StatusBar />}
      {!isProjectsWindow && <DebugToolbar />}
      <CommandPalette />
      <SearchEverywhere />
      <NewProjectDialog />
      <FormDialog />
