...
/** Focus in the code itself — not in the search bar or another of the editor's panels. */
const inEditor = (target: EventTarget | null) => {
  const el = target as HTMLElement | null
  return Boolean(el?.closest?.('.cm-editor') && !el.closest('.cm-panels'))
}
...
/** The global shortcuts — every binding comes from the shortcut system. */
