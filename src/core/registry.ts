...
class Registry {
  ...
  unregister(id: string): boolean {
    ...
    if (!addon || addon.builtin) return false
    this.deactivate(id)
  ...
  replace(addon: Addon) {
    ...
    if (wasActive) this.deactivate(addon.id)
    this.addons.set(addon.id, addon)
    if (wasActive) this.activate(addon.id)
    this.emit()
  }
  ...
  activate(id: string): boolean {
    const addon = this.addons.get(id)
    if (!addon || this.active.has(id)) return false
    this.active.add(id)
    try {
  ...
  deactivate(id: string): boolean {
    const addon = this.addons.get(id)
    if (!addon || addon.builtin || !this.active.has(id)) return false
    try {
  ...
  applyEnabled(ids: string[]) {
    ...
    for (const addon of this.addons.values()) {
      ...
      if (should && !active) this.activate(addon.id)
      if (!should && active) this.deactivate(addon.id)
    }
  }
