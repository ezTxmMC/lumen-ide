...
export function packageFits(spec: LspPackage, platformKey: string | undefined): boolean {
  if (spec.type === 'github') return Boolean(spec.assets[platformKey ?? ''])
  if (spec.type === 'archive' && typeof spec.url !== 'string') return Boolean(spec.url[platformKey ?? ''])
  return true
}
...
export function installPlans(config: LspConfig, ctx: PlanContext): InstallPlan[] {
  ...
  for (const manager of ctx.system?.managers ?? []) {
    if (!isPackageManager(manager)) continue
...
export function planKey(plan: InstallPlan): string {
  if (plan.kind === 'managed') return 'managed'
  if (plan.kind === 'system') return `system:${plan.manager}`
  return 'command'
}
