...
export function toMarkdown(
  ...
  if (!value) return ''
  if (typeof value === 'string') return value
  if (Array.isArray(value)) return value.map(toMarkdown).join('\n\n')
  if ('kind' in value) return value.kind === 'plaintext' ? escapeMarkdown(value.value) : value.value
...
export function pathToUri(filePath: string): string {
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(filePath) && !filePath.startsWith('file://')) return filePath
