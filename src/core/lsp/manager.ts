...
class LspManager {
  ...
  private async resolveConfig(
    ...
    if (spec.lsp?.length) {
      this.missing.set(spec.id, this.orderedConfigs(spec)[0]);
    }
    return null;
  }
  ...
  private async withInvisibleProjectRoots(config: LspConfig, root: string): Promise<LspConfig> {
    ...
    if (builds.some(Boolean) || !(await has('src'))) {
      return config;
    }
  ...
  private async ensure(spec: LanguageSpec, filePath: string): Promise<LspClient | null> {
    if (!this.enabled || !spec.lsp?.length) {
      return null;
    }
    ...
    const task = (async () => {
      ...
      if (existing) {
        ...
        if (existing.status === 'starting' && startup) {
          return (await startup) ? existing : null;
        }
      ...
      for (const decorate of this.decorators) {
        decorated = await decorate(decorated, spec.id, actualRoot, command);
      }
      if (isJdtls(decorated)) {
        decorated = await this.withInvisibleProjectRoots(decorated, actualRoot);
      }
  ...
  private wire(client: LspClient) {
    ...
    client.onJavaEvent = (event) => {
      if (event.kind === 'projects') {
        ...
        if (this.staleImports.has(client)) {
          void this.reimportStale(client);
        }
        return;
      }
      ...
      if (this.importWarned.has(client)) {
        return;
      }
  ...
  private scheduleResync(client: LspClient) {
    ...
    this.resyncTimers.set(client, setTimeout(() => {
      ...
      for (const path of client.openPaths()) {
        ...
        if (doc) {
          client.reopenDocument(path, doc.text);
        }
      }
    }, 1200));
  }
  ...
  private async prepareJava(client: LspClient) {
    ...
    if (cleaned.kept.length) {
      client.addLog('client', 2, t('lsp.java.metadataTracked', { files: cleaned.kept.join(', ') }));
    }
    const state = await window.lumen.lsp.javaImportState(dataDir).catch(() => 'current');
    if (state === 'stale') {
      this.staleImports.add(client);
    }
  }
  ...
  async applyPreference(spec: LanguageSpec, root: string | null) {
    ...
    if (root) {
      await this.startForProject(spec, root);
    }
    this.emit();
  }
  ...
  clientForLanguage(languageId: string): LspClient | null {
    for (const client of this.clients.values()) {
      if (client.status === 'ready' && client.servedLanguages.includes(languageId)) {
        return client;
      }
    }
    return null;
  ...
  notifyFsChanges(changes: FsChange[]) {
    ...
    for (const client of this.clients.values()) {
      ...
      if (!client.watchedPatterns.length && !java) {
        continue;
      }
      ...
      if (changes.some(unwatched)) {
        this.scheduleJavaReimport(client);
      }
  ...
  status(spec: LanguageSpec | null): LspStatus {
    ...
    if (!this.enabled) {
      return { status: 'stopped', label: t('lsp.off'), detail: '' };
    }
  ...
  anyBusy(): string | null {
    for (const client of this.clients.values()) {
      ...
      if (busy) {
        return `${client.config.label.split(/[\s-]/)[0]}: ${busy}`;
      }
    }
    return null;
  }
  ...
  logs(serverId?: string): (LogLine & { server: string; })[] {
    ...
    for (const client of this.clients.values()) {
      if (serverId && client.id !== serverId) {
        continue;
      }
      for (const line of client.log) {
        out.push({ ...line, server: client.config.label });
      }
    }
  ...
  async restart(spec: LanguageSpec | null, filePath: string | null) {
    ...
    if (client) {
      await this.restartClient(client.id);
    }
    if (!client) {
      this.rescan();
    }
    ...
    if (doc) {
      await this.openDocument(spec, filePath, doc.text);
    }
  }
  ...
  private deadClientFor(spec: LanguageSpec): LspClient | null {
    ...
    for (const client of this.clients.values()) {
      if (client.status === 'ready' || client.status === 'starting') {
        continue;
      }
      if (commands.has(client.config.command)) {
        return client;
      }
    }
    return null;
