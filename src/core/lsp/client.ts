...
export class LspClient {
  ...
  private handleNotification(method: string, params: never) {
    switch (method) {
      ...
      case 'language/status': {
        ...
        if (p.type === 'Error') {
          this.addLog('log', 1, p.message);
        }
        if (p.type !== 'Error' && p.type !== 'ProjectStatus') {
          this.addLog('log', 3, `${p.type}: ${p.message}`);
        }
        ...
        if (p.type === 'Starting' && !this.javaReady) {
          this.progress.set('java-status', { title: 'Java', message: p.message });
        }
        if (p.type === 'ServiceReady') {
          this.javaReady = true;
        }
        if (p.type === 'ServiceReady' || p.type === 'Started') {
          this.progress.delete('java-status');
        }
        if (p.type === 'ServiceReady') {
          this.onJavaEvent({ kind: 'projects' });
        }
        // `ProjectStatus: WARNING` — at least one project did not import cleanly.
        if (p.type === 'ProjectStatus' && p.message !== 'OK') {
          this.onJavaEvent({ kind: 'importProblem', message: p.message });
        }
        this.onStatusChange();
        return;
      }
      case 'language/eventNotification': {
        ...
        if (p.eventType === 100 || p.eventType === 200) {
          this.onJavaEvent({ kind: 'projects' });
        }
        if (p.eventType === 300) {
          this.onJavaEvent({ kind: 'gradleJdk', message: JSON.stringify(p.data ?? '') });
        }
        return;
      }
  ...
  private async handleRequest(method: string, params: never): Promise<unknown> {
    switch (method) {
      ...
      case 'window/showDocument': {
        ...
        if (p.external && /^https?:/.test(p.uri)) {
          void window.lumen.shell.openExternal(p.uri);
        }
        return { success: true };
      }
  ...
  private executeClientCommand(params: { command: string; arguments?: unknown[]; }): unknown {
    if (params.command === 'java.action.organizeImports.chooseImports') {
      return this.chooseImports(params.arguments ?? []);
    }
  ...
  notify(method: string, params: unknown) {
    if (this.status !== 'ready' && method !== 'initialized') {
      return;
    }
  ...
  openDocument(filePath: string, text: string, languageId: string) {
    if (this.status !== 'ready' || this.openDocs.has(filePath)) {
      return;
    }
  ...
  async javaReimport(buildFiles: string[], options: { full?: boolean; } = {}) {
    for (const file of buildFiles) {
      this.notify('java/projectConfigurationUpdate', { uri: pathToUri(file) });
    }
  ...
  closeDocument(filePath: string) {
    if (!this.openDocs.delete(filePath)) {
      return;
    }
  ...
  didChangeWatchedFiles(changes: { uri: string; type: 1 | 2 | 3; }[], force = false) {
    if (!changes.length || (!force && !this.watchedPatterns.length)) {
      return;
    }
  ...
  async completionResult(
    ...
    if (!this.supports('completionProvider')) {
      return { ok: true, list: { isIncomplete: false, items: [] } };
    }
  ...
  async completion(filePath: string, position: Position, trigger?: string, signal?: AbortSignal, triggerKind?: 1 | 2 | 3): Promise<CompletionList> {
    ...
    if (!outcome.cancelled) {
      this.addLog('client', 2, `completion: ${outcome.error.message}`);
    }
  ...
  resolveCompletionResult(item: CompletionItem): Promise<CompletionOutcomeResolve> {
    ...
    if (cached) {
      return Promise.resolve({ ok: true, item: cached });
    }
  ...
  private async resolveOnce(item: CompletionItem, key: string): Promise<CompletionOutcomeResolve> {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        if (this.resolved.size > RESOLVE_CACHE) {
          this.resolved.delete(this.resolved.keys().next().value as string);
        }
  ...
  declaration(filePath: string, position: Position) {
    if (!this.supports('declarationProvider')) {
      return Promise.resolve([] as Location[]);
    }
  ...
  implementation(filePath: string, position: Position) {
    if (!this.supports('implementationProvider')) {
      return Promise.resolve([] as Location[]);
    }
  ...
  async documentHighlight(filePath: string, position: Position) {
    if (!this.supports('documentHighlightProvider')) {
      return [];
    }
  ...
  async signatureHelp(filePath: string, position: Position, trigger?: string, isRetrigger = false) {
    if (!this.supports('signatureHelpProvider')) {
      return null;
    }
  ...
  async formatting(filePath: string, options: FormattingOptions) {
    if (!this.supports('documentFormattingProvider')) {
      return null;
    }
  ...
  async rangeFormatting(filePath: string, range: Range, options: FormattingOptions) {
    if (!this.supports('documentRangeFormattingProvider')) {
      return null;
    }
  ...
  async codeActions(filePath: string, range: Range, diagnostics: Diagnostic[], only?: string[]) {
    if (!this.supports('codeActionProvider')) {
      return [];
    }
  ...
  async resolveCodeAction(action: CodeAction): Promise<CodeAction> {
    ...
    if (!provider?.resolveProvider || action.edit) {
      return action;
    }
  ...
  async executeCommand(command: LspCommand): Promise<unknown> {
    if (!this.supports('executeCommandProvider')) {
      return null;
    }
  ...
  async inlayHints(filePath: string, range: Range): Promise<InlayHint[]> {
    if (!this.supports('inlayHintProvider')) {
      return [];
    }
