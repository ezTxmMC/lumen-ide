...
function remoteChoices(field: UserTemplateField): Choice[] | undefined {
  if (!field.choicesUrl) return undefined
...
async function refreshRemoteChoices(model: UserAddonModel, force = false) {
  ...
  for (const url of urls) {
    ...
    if (!force && known && Date.now() - known.fetchedAt < CHOICES_TTL_MS) continue
    if (pending.has(url) || !/^https:\/\//.test(url)) continue
    pending.add(url)
    try {
  ...
  if (!changed || !current || !registry.get(model.id)) return
...
function refreshStore() {
  ...
  if (!registry.themes().some((theme) => theme.id === s.themeId)) s.setTheme(DEFAULT_THEME_ID)
  s.applyKeybindings()
}
...
function registerModel(model: UserAddonModel, activate: boolean) {
  ...
  if (activate) registry.activate(model.id)
  void refreshRemoteChoices(model)
}
...
export const userAddons = {
  ...
  async init() {
    ...
    for (const id of enabled) {
      if (models.has(id) && !registry.isActive(id)) registry.activate(id)
    }
  ...
  async save(input: UserAddonModel, previousId?: string | null): Promise<ValidationIssue[]> {
    ...
    if (blockingIssues(issues).length) return issues
    const taken = registry.get(model.id)
    if (taken && !taken.user) return [{ section: 'general', field: 'id', message: t('addonStudio.validate.duplicateId', { id: model.id }) }]
    ...
    if (isNew) setEnabled(model.id, activate)
    refreshStore()
    emit()
    return issues
  },
  ...
  async duplicate(id: string): Promise<UserAddonModel | null> {
    ...
    if (blockingIssues(issues).length) return null
    return copy
  },
  ...
  serialize(model: UserAddonModel): string {
    ...
    const themes = model.themes.map((entry) => {
      if ('theme' in entry) return entry
  ...
  async exportModel(model: UserAddonModel) {
    ...
    if (target) useStore.getState().notify(t('addonStudio.manager.exported', { name: model.name }), 'success')
  },
  ...
  async importFile(): Promise<UserAddonModel | null> {
    ...
    if (!model.id.startsWith(USER_ADDON_PREFIX)) model.id = uniqueAddonId(slugify(model.id || model.name), existing)
    const replace = models.has(model.id) && confirm(t('addonStudio.manager.confirmReplace', { name: model.name || model.id }))
    if (registry.get(model.id) && !replace) model.id = uniqueAddonId(model.id, existing)
...
function languageFromSpec(spec: LanguageSpec, taken: Set<string>): UserLanguage {
  ...
  let n = 2
  while (taken.has(id)) id = `${spec.id}-custom${n++}`
  taken.add(id)
