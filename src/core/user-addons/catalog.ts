  ...
  return {
    ...
    run(ctx) {
      for (const out of outputs) {
        ctx.output(out.id, ctx.payload[out.id] ?? '');
      }
      return 'then';
    },
  };
...
const NODES: NodeDef[] = [
  ...
  pure('text.template', 'text', [str('template', '{a}'), any('a'), any('b'), any('c')], [str('result')], async (ctx) => {
    ...
    for (const id of ['a', 'b', 'c']) {
      if (template.includes(`{${id}}`)) {
        values[id] = toText(await ctx.input(id));
      }
    }
  ...
  pure('text.replace', 'text', [str('text', ''), str('pattern', ''), str('replacement', '')], [str('result')], async (ctx) => {
    ...
    if (ctx.setting('mode') === 'plain') {
      return { result: source.split(pattern).join(replacement) };
    }
    let regex: RegExp;
    try {
  ...
  pure('math.divide', 'math', [num('a', 0), num('b', 1)], [num('result')], async (ctx) => {
    ...
    if (b === 0) {
      throw new GraphError(t('addonStudio.run.divideByZero'), ctx.node.id);
    }
