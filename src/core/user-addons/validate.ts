...
export function validateGraph(graph: Graph): string[] {
  ...
  for (const node of graph.nodes) {
    if (!NODE_CATALOG.has(node.type)) {
      problems.push(v('unknownNode', { type: node.type }));
    }
  }
...
/** Checks one language definition. */
function validateLanguage(lang: UserAddonModel['languages'][number], index: number, push: Push) {
  ...
  if (!LANGUAGE_ID_PATTERN.test(lang.id)) {
    at('id', v('languageId'));
  }
  if (!lang.name.trim()) {
    at('name', v('nameRequired'));
  }
  if (!lang.extensions.length && !lang.filenames?.length) {
    at('extensions', v('extensionsRequired'));
  }
  ...
  if (block && (!block[0] || !block[1])) {
    at('comments', v('blockComment'));
  }
  lang.strings?.forEach((rule) => {
    if (!rule.start) {
      at('strings', v('stringStart'));
    }
  });
  ...
  for (const name of lang.debug ?? []) {
    if (BUILTIN_DEBUG_ADAPTERS[name]) {
      continue;
    }
...
/** The add-on's own identity. */
function validateGeneral(model: UserAddonModel, push: Push) {
  if (!ADDON_ID_PATTERN.test(model.id)) {
    push({ section: 'general', field: 'id', message: v('addonId', { prefix: USER_ADDON_PREFIX }) });
  }
  if (!model.name.trim()) {
    push({ section: 'general', field: 'name', message: v('nameRequired') });
  }
  if (!VERSION_PATTERN.test(model.version)) {
    push({ section: 'general', field: 'version', message: v('version') });
  }
  if (model.color && !COLOR_PATTERN.test(model.color)) {
    push({ section: 'general', field: 'color', message: v('color') });
  }
}

/** Languages, with duplicate ids across them. */
function validateLanguages(model: UserAddonModel, push: Push) {
  ...
  model.languages.forEach((lang, index) => validateLanguage(lang, index, push));
}

/** Commands and their graphs. */
function validateCommands(model: UserAddonModel, push: Push) {
  ...
  model.commands.forEach((command, index) => {
    ...
    if (!LOCAL_ID_PATTERN.test(command.id)) {
      at(v('localId'), 'id');
    }
    if (!command.title.trim()) {
      at(v('titleRequired'), 'title');
    }
    ...
    if (entries > 1) {
      at(v('commandEntryOnce'));
    }
    if (command.keybinding && !/^[^\s+]+(?:\+[^\s+]+)*(?: [^\s+]+(?:\+[^\s+]+)*)?$/.test(command.keybinding)) {
      at(v('keybinding'), 'keybinding');
    }
    for (const problem of validateGraph(command.graph)) {
      at(problem);
    }
  });
}

/** Event handlers and their graphs. */
function validateEvents(model: UserAddonModel, push: Push) {
  model.events.forEach((event, index) => {
    ...
    if (!entries.length) {
      at(v('eventEntry'), true);
    }
    for (const problem of validateGraph(event.graph)) {
      at(problem);
    }
  });
}

/** Checks one project template. */
function validateTemplate(template: UserAddonModel['templates'][number], index: number, push: Push) {
  const at = (message: string, field?: string) =>
    push({ section: 'templates', index, field, message: `${template.name || template.id}: ${message}` });
  if (!LOCAL_ID_PATTERN.test(template.id)) {
    at(v('localId'), 'id');
  }
  if (!template.name.trim()) {
    at(v('nameRequired'), 'name');
  }
  if (!template.files.length) {
    at(v('templateFiles'), 'files');
  }
  ...
  for (const field of template.fields) {
    if (!FIELD_ID_PATTERN.test(field.id)) {
      at(v('fieldId', { id: field.id }), 'fields');
    }
    if (['name', 'slug', 'dir'].includes(field.id)) {
      at(v('fieldReserved', { id: field.id }), 'fields');
    }
    const error = checkRegex(field.pattern);
    if (error) {
      at(v('regex', { field: field.label || field.id, error }), 'fields');
    }
    if (field.type === 'select' && !field.choices?.length && !field.choicesUrl) {
      at(v('fieldChoices', { id: field.id }), 'fields');
    }
    if (field.choicesUrl && !/^https:\/\/\S+$/.test(field.choicesUrl)) {
      at(p('choicesUrl', { id: field.id }), 'fields');
    }
    const matchError = checkRegex(field.choicesMatch);
    if (matchError) {
      at(v('regex', { field: field.label || field.id, error: matchError }), 'fields');
    }
  ...
  for (const file of template.files) {
    ...
    for (const condition of conditions) {
      ...
      if (template.fields.some((other) => other.id === condition.field)) {
        continue;
      }
...
/** Project templates. */
function validateTemplates(model: UserAddonModel, push: Push) {
  ...
  model.templates.forEach((template, index) => validateTemplate(template, index, push));
}

/** Checks one project kind. */
function validateKind(kind: NonNullable<UserAddonModel['projectKinds']>[number], index: number, push: Push) {
  const at = (message: string, field?: string) =>
    push({ section: 'kinds', index, field, message: `${kind.name || kind.id}: ${message}` });
  if (!LOCAL_ID_PATTERN.test(kind.id)) {
    at(v('localId'), 'id');
  }
  if (!kind.name.trim()) {
    at(v('nameRequired'), 'name');
  }
  if (!kind.markers.some((marker) => marker.trim())) {
    at(p('kindMarkers'), 'markers');
  }
  for (const rule of kind.rules ?? []) {
    ...
    if (error) {
      at(v('regex', { field: rule.file, error }), 'rules');
    }
  }
  ...
  for (const task of kind.tasks) {
    if (!task.label.trim() || !task.command.trim()) {
      at(p('taskIncomplete'), 'tasks');
    }
  }
  ...
  for (const fact of kind.facts ?? []) {
    ...
    if (fact.json !== undefined) {
      if (!fact.file.trim()) {
        at(p('factPattern', { label: fact.label || fact.file }), 'facts');
      }
      continue;
    }
    ...
    if (!fact.file.trim() || !/\((?!\?[:=!<])/.test(fact.pattern ?? '')) {
      at(p('factPattern', { label: fact.label || fact.file }), 'facts');
    }
  }
}
...
/** Project kinds. */
function validateKinds(model: UserAddonModel, push: Push) {
  ...
  kinds.forEach((kind, index) => validateKind(kind, index, push));
}

/** Snippets. */
function validateSnippets(model: UserAddonModel, push: Push) {
...
/** Panels. */
function validatePanels(model: UserAddonModel, push: Push) {
  const panelIds = new Set<string>();
  (model.panels ?? []).forEach((panel, index) => {
    if (!/^[a-z][a-z0-9-]*$/.test(panel.id)) {
      push({ section: 'panels', index, field: 'id', message: p('panelId', { id: panel.id || String(index + 1) }) });
    }
    if (panelIds.has(panel.id)) {
      push({ section: 'panels', index, field: 'id', message: p('panelDuplicate', { id: panel.id }) });
    }
    panelIds.add(panel.id);
    if (!panel.title.trim() || !panel.content.trim()) {
      push({ section: 'panels', index, message: p('panelEmpty', { id: panel.id || String(index + 1) }) });
    }
  });
}

/** Bundled themes. */
function validateThemes(model: UserAddonModel, push: Push) {
  model.themes.forEach((entry, index) => {
    if ('ref' in entry) {
      if (!entry.ref) {
        push({ section: 'themes', index, message: v('themeRef') });
      }
      return;
    }
    ...
    if (!valid) {
      push({ section: 'themes', index, message: v('themeInvalid') });
    }
  });
}
...
export function validateAddon(model: UserAddonModel): ValidationIssue[] {
  ...
  const push: Push = (issue) => issues.push(issue);

  validateGeneral(model, push);
  validateLanguages(model, push);
  validateCommands(model, push);
  validateEvents(model, push);
  validateTemplates(model, push);
  validateKinds(model, push);
  validateSnippets(model, push);
  validatePanels(model, push);
  validateThemes(model, push);
