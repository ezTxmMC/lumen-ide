...
export function resolveWorkspacePath(input: string): string {
  ...
  if (!root) throw new Error(t('addonStudio.run.noWorkspace'))
  if (!input.trim()) throw new Error(t('addonStudio.run.emptyPath'))
  const base = normalize(root)
  const target = normalize(isAbsolute(input) ? input : `${root}/${input}`)
  if (target !== base && !target.startsWith(`${base}/`)) throw new Error(t('addonStudio.run.outsideWorkspace', { path: input }))
...
function formPromise(spec: Parameters<ReturnType<typeof useStore.getState>['openForm']>[0], pickValue: (values: Record<string, string>) => string) {
  return new Promise<string | null>((resolve) => {
    ...
    const unsubscribe = useStore.subscribe((state) => {
      if (state.formDialog !== wrapped) finish(null)
    })
...
export const rendererHost: GraphHost = {
  ...
  replaceSelection(text) {
    const v = view()
    if (!v) throw new Error(t('addonStudio.run.noEditor'))
  ...
  cursor() {
    const v = view()
    if (!v) return { line: 0, column: 0 }
    const head = v.state.selection.main.head
    const line = v.state.doc.lineAt(head)
    return { line: line.number, column: head - line.from + 1 }
  },
  gotoLine(line) {
    if (!view()) throw new Error(t('addonStudio.run.noEditor'))
  ...
  async shell(addonId, command) {
    if (!command.trim()) throw new Error(t('addonStudio.run.emptyCommand'))
    if (!shellAllowed(addonId)) {
      const name = registry.get(addonId)?.name ?? addonId
      if (!confirm(t('addonStudio.run.confirmShell', { name, command }))) throw new Error(t('addonStudio.run.shellDenied'))
      allowShell(addonId)
    }
    const s = useStore.getState()
  ...
  setTheme(id) {
    if (!registry.themes().some((theme) => theme.id === id)) throw new Error(t('addonStudio.run.unknownTheme', { id }))
    useStore.getState().setTheme(id)
  },
  toggleSetting(key, mode) {
    if (!TOGGLE_SETTINGS.includes(key)) throw new Error(t('addonStudio.run.unknownSetting', { key }))
    const s = useStore.getState()
    ...
    ? useStore.subscribe((state, prev) => {
      if (state.tabs !== prev.tabs) {
        ...
        for (const tab of state.tabs) {
          ...
          if (savedNow && wasDirty) fire(model, 'fileSaved', pathPayload(tab.path, tab.languageId))
        }
      }
