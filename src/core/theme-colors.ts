...
export function toHex({ r, g, b, a }: Rgba): string {
  ...
  if (Math.round(clamp(a) * 255) >= 255) return base
  return base + byte(a)
}
...
export function maxChroma(l: number, h: number, upper = 0.4): number {
  const light = clamp(l)
  if (inGamut(oklchToOklab({ l: light, c: upper, h }))) return upper
  let low = 0
  let high = upper
  for (let i = 0; i < 24; i++) {
...
export function oklchToRgb(lch: Oklch, alpha = 1): Rgba {
  ...
  if (inGamut(full)) return oklabToRgb(full, alpha)
...
export function ensureContrast(fg: string, backgrounds: string | string[], min: number): string {
  ...
  const search = (target: number): { color: string; ok: boolean; distance: number } => {
    const end = at(target)
    if (worst(end) < min) return { color: end, ok: false, distance: Math.abs(target - lch.l) }
    let near = lch.l
    let far = target
    for (let i = 0; i < 28; i++) {
  ...
  if (reached.length) return reached[0].color
...
export function relight(lch: Oklch, l: number): Oklch {
  ...
  if (available < 1e-6) return { l: target, c: 0, h: lch.h }
...
export function syntaxMinContrast(kind: TokenKind): number {
  if (kind === 'comment' || kind === 'punctuation') return COMMENT_MIN_CONTRAST
...
export function invertTheme(theme: Theme): Theme {
  ...
  const ui = { ...theme.ui }
  for (const key of UI_COLOR_KEYS) ui[key] = invertAround(theme.ui[key], bgL)
...
export function contrastIssues(theme: Theme): ContrastIssue[] {
  ...
  const check = (key: string, fg: string, against: UIColorKey, min: number) => {
    ...
    if (ratio < min) issues.push({ key, against, ratio, min })
  }
  for (const [key, min] of Object.entries(UI_MIN_CONTRAST) as [UIColorKey, number][]) {
    for (const against of ['bg', 'bgElevated', 'bgOverlay'] as const) check(`ui:${key}`, theme.ui[key], against, min)
  }
  check('ui:accentText', theme.ui.accentText, 'accent', 4.5)
  for (const kind of TOKEN_KINDS) {
    if (!theme.syntax[kind]) continue
  ...
  for (const issue of issues) {
    ...
    if (known && known.ratio <= issue.ratio) continue
