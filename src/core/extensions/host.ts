...
export function parseViewTabPath(path: string | null | undefined): { extensionId: string; viewId: string; instance: string } | null {
  if (!path?.startsWith(VIEW_TAB_SCHEME)) return null
...
function closeViewTabs(tabs: OpenTab[], previous: OpenTab[]) {
  ...
  for (const tab of previous) {
    ...
    if (!view || still.has(tab.path)) continue
...
function activeFileEvent(state: StoreState): HostEvent {
  ...
  if (!tab?.path || tab.virtual) return { kind: 'activeFile', path: null }
...
function watchStore() {
  ...
  for (const [id, values] of Object.entries(lastSettings)) void window.lumen.extensionHost.setSettings(id, values)
  ...
  if (!document.hasFocus()) sendEvent({ kind: 'windowBlur' })

  useStore.subscribe((state, previous) => {
    if (state.extensionSettings !== lastSettings) {
      for (const [id, values] of Object.entries(state.extensionSettings)) {
        if (lastSettings[id] !== values) void window.lumen.extensionHost.setSettings(id, values)
      }
    ...
    if (state.tabs !== previous.tabs) closeViewTabs(state.tabs, previous.tabs)
    if (state.activeTabId !== previous.activeTabId) sendEvent(activeFileEvent(state))
    if (state.project !== previous.project || state.workspace !== previous.workspace) sendEvent(projectEvent(state))
    if (state.tabs === previous.tabs) return
    for (const tab of state.tabs) {
      const before = previous.tabs.find((open) => open.id === tab.id)
      if (!before || before.saved === tab.saved || !tab.path || tab.virtual) continue
...
export const extensionHost = {
  ...
  show(extensionId: string, viewId: string, instance?: string): () => void {
    ...
    if (stateOf(key).stale || !stateOf(key).content) void load(extensionId, viewId, instance)
    ...
    return () => {
      ...
      if (count <= 0) visible.delete(key)
      if (count > 0) visible.set(key, count)
    }
  },
  ...
  init() {
    ...
    host.onViewChanged(({ extensionId, viewId, instance }) => {
      ...
      for (const key of new Set([...viewStates.keys(), ...visible.keys()])) {
        if (key.startsWith(prefix)) changed(key, key.slice(prefix.length))
      }
    })
