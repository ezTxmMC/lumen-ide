...
export function activeDockViewId(): string | null {
  ...
  for (const dock of docks) {
    if (!layout[dock].open) continue
    const id = activeView(layout, dock, known)
    if (id && !isViewPopped(popouts, id)) return id
  }
  return null
}
...
export function dockBackTargetKey(): string | null {
  ...
  if (focused && popouts.some((entry) => entry.key === focused)) return focused
  if (poppedGroupIds(popouts).has(activeGroupId)) return popoutKey('group', activeGroupId)
