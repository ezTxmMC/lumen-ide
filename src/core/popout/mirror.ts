...
function copyAttributes(from: Element, to: Element) {
  for (const { name } of [...to.attributes]) if (!from.hasAttribute(name)) to.removeAttribute(name)
...
function mirrorStyles(source: Document, target: Document): () => void {
  ...
  const sync = () => {
    ...
    for (const [node, clone] of clones) {
      if (current.includes(node)) continue
      clone.remove()
      clones.delete(node)
    }
    for (const node of current) {
      ...
      if (node instanceof HTMLStyleElement && known.textContent !== node.textContent) known.textContent = node.textContent
      if (node instanceof HTMLLinkElement && (known as HTMLLinkElement).href !== node.href) (known as HTMLLinkElement).href = node.href
    }
    ...
    for (const node of current) {
      ...
      if (!clone) continue
      if (previous && previous.nextElementSibling !== clone) previous.after(clone)
      previous = clone
    }
  }
  ...
  return () => {
    observer.disconnect()
    for (const clone of clones.values()) clone.remove()
    clones.clear()
  }
}
