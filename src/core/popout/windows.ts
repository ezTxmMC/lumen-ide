...
function release(key: string) {
  ...
  for (const fn of closedListeners) fn(key)
}

function watch() {
  if (watcher) return
  watcher = setInterval(() => {
    for (const [key, win] of open) if (win.closed) release(key)
  }, WATCH_MS)
}
...
export function closeAllPopoutWindows() {
  for (const key of [...open.keys()]) closePopoutWindow(key)
}
...
export function saveBounds(key: string, bounds: PopoutBounds) {
  ...
  if (!boundsChanged(all[slot] ?? null, bounds)) return
  try {
...
if (typeof window.addEventListener === 'function') window.addEventListener('pagehide', closeAllPopoutWindows)
