...
function distributionOf(jdk: DetectedJdk): string {
  if (jdk.distribution) return jdk.distribution
...
export function catalogUrl(platform: string, arch: string, options: CatalogOptions): string {
  ...
  for (const d of JAVA_DISTRIBUTIONS.filter((entry) => entry.downloadable)) params.append('distribution', d.id)
  ...
  if (options.earlyAccess) params.append('release_status', 'ea')
  if (platform === 'linux') params.set('lib_c_type', 'glibc')
...
export function normalizePackages(raw: DiscoPackage[]): SdkPackage[] {
  ...
  for (const entry of raw) {
    if (entry.major_version < MIN_MAJOR || !entry.directly_downloadable) continue
    if (entry.release_status === 'ea' && entry.major_version <= newestGa) continue
    if (entry.size > 0 && entry.size < MIN_SIZE) continue
    ...
    if (known && compareVersions(known.version, pkg.version) >= 0) continue
    best.set(key, pkg)
  }
...
export function jdtlsRuntimeName(major: number): string {
  if (major <= 8) return `JavaSE-1.${major}`
  return `JavaSE-${major}`
}
