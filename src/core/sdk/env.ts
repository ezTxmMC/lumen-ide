...
function compose(): Record<string, string> {
  if (!base || !active.length) return {}
  const env: Record<string, string> = {}
  for (const sdk of active) Object.assign(env, sdk.variables)
...
export function resolveSdk(value: string | undefined, installed: InstalledSdk[]): Pick<InstalledSdk, 'home' | 'major'> | null {
  ...
  if (major) return byNewest(usable.filter((sdk) => sdk.major === Number(major[1])))
