...
export function setDefaultSdk(providerId: string, value: string | null) {
  ...
  if (value) defaults[providerId] = value
  if (!value) delete defaults[providerId]
...
export async function installPackage(pkg: SdkPackage) {
  ...
  if (running && !isFinished(running.progress.phase)) return
...
export async function removeInstalled(sdk: InstalledSdk) {
  ...
  if (settings.defaults[sdk.providerId] === sdk.home) await setDefaultSdk(sdk.providerId, null)
  if (useStore.getState().projectConfig.jdk === sdk.home) await setProjectSdk(null)
