...
function focusInEditorField(view: EditorView): boolean {
  ...
  if (!active || active === view.contentDOM || !view.dom.contains(active)) return false
...
export function buildCommands(options: { includeHidden?: boolean } = {}): Command[] {
  ...
  /** The active file, with its language server attached. */
  const editorPath = () => {
    const tab = s().activeTab()
    return tab?.path && !tab.virtual && editorBridge.view ? tab.path : null
  }
  const withEditor = (fn: (view: NonNullable<typeof editorBridge.view>, path: string) => unknown) => () => {
    const path = editorPath()
    if (path && editorBridge.view) void fn(editorBridge.view, path)
  }
  ...
  const core: Command[] = [
    ...
    },
    {
      ...
      run: () => {
        ...
        if (p && file) void s().openFile(`${p.root}/${file}`)
      },
    ...
    { id: 'view.addons', title: c('view.addons'), category: cat.view, run: () => s().openDialog('addons') },
    ...
    ...Object.entries(EDITOR_COMMANDS).map<Command>(([id, command]) => ({
      ...
      run: () => {
        ...
        if (focusInEditorField(view)) return
        view.focus()
      },
    })),
  ...
  const shellCommands: Command[] = terminals.shells.map((shell) => ({
  ...
  const externalCommands: Command[] = terminals.externals.map((term) => ({
  ...
  const themeCommands: Command[] = registry.themes().map((theme) => ({
    id: `theme.${theme.id}`,
  ...
  const runCommands: Command[] = (language?.run ?? []).map((cfg) => ({
  ...
  const taskCommands: Command[] = [...s().projectConfig.tasks, ...(s().project?.tasks ?? [])].map((task) => ({
    id: `task.${task.id}`,
  ...
  const provided: Command[] = [...providers].flatMap((provider) => {
    try {
      return provider()
    } catch (err) {
...
function editorCategory(id: string, cat: Record<string, string>) {
  if (/fold/i.test(id)) return cat.folding
  if (/select|cursor|Occurrence/i.test(id)) return cat.selection
  return cat.editor
}
