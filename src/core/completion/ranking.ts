...
export function matchTier(q: Query, p: Prepared, errors: number): number {
  ...
  if (p.text.length >= q.n) {
    if (p.text.startsWith(q.text)) {
      return 0;
    }
    let i = 0;
    while (i < q.n && p.lower[i] === q.lower[i]) {
      i++;
    }
    if (i === q.n) {
      return 1;
...
function isHump(q: Query, p: Prepared): boolean {
  ...
  for (let i = 0; i < q.n; i++) {
    ...
    let k = at + 1;
    while (k < p.lower.length && !(p.flags[k] & 3 && p.lower[k] === q.lower[i])) {
      k++;
    }
    if (k >= p.lower.length) {
      return false;
...
export class RankCache {
  ...
  candidates(pool: readonly Candidate[], q: Query): PoolCache | null {
    ...
    if (hit.maxErrors !== q.maxErrors || hit.firstError !== q.firstError) {
      return null;
    }
    if (!q.text.toLowerCase().startsWith(hit.pattern.toLowerCase())) {
      return null;
    }
    return hit;
  }
...
export function rank<T>(
  ...
  for (const pool of pools) {
    ...
    for (let k = 0; k < total; k++) {
      ...
      if (signals.memberAccess && HIDDEN_ON_MEMBER.has(candidate.origin)) {
        continue;
      }
      if (!accepts(q, raw, errors)) {
        continue;
      }

      let group = ORIGIN_BOOST[candidate.origin] + candidate.boost;
      if (signals.statementStart) {
        group += STATEMENT_BOOST[candidate.origin] ?? 0;
      }
      if (lowerStart && candidate.kind === 'variable') {
        group += VARIABLE_ON_LOWER;
      }
      if (lowerStart && candidate.kind === 'type') {
        group += TYPE_ON_LOWER;
      }
...
function dropShadowed<T>(best: Map<string, Ranked<T>>): Ranked<T>[] {
  ...
  for (const entry of best.values()) {
    if (entry.candidate.identity === undefined) {
      continue;
    }
