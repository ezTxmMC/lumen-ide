...
function parseType(text: string): ParsedType | null {
  ...
  if (!/^[A-Za-z_$][\w$]*$/.test(base)) return null
  const args: string[] = []
...
export function nameSuggestions(typeText: string): string[] {
  ...
  add(own)
  if (words.length > 1) add(words[words.length - 1].toLowerCase())
  if (elementName && COLLECTIONS.has(base)) {
    add(pluralize(elementName))
    add(`${elementName}${words[words.length - 1]}`)
  }
  if (elementName && SINGLE_HOLDERS.has(base)) add(elementName)
  if (elementName && base === 'Map' && args[1]) {
    const value = simple(args[1])
    if (/^[A-Z]/.test(value)) add(`${elementName}To${value}`.replace(/^./, (c) => c.toLowerCase()))
  }
  if (elementName && !COLLECTIONS.has(base) && !SINGLE_HOLDERS.has(base)) add(`${elementName}${base}`)
  return out
}
...
export function declaredTypeBefore(lineBefore: string, languageId: string | null | undefined): string | null {
  if (!languageId || !TYPE_FIRST.has(languageId)) return null
  ...
  if (head === '') return match[1]
  const last = head.slice(-1)
  if ('(,;{}'.includes(last)) return match[1]
  const word = /([A-Za-z_$][\w$]*)$/.exec(head)?.[1]
  if (word && MODIFIERS.has(word)) return match[1]
  if (/@[\w$.]+(?:\([^)]*\))?$/.test(head)) return match[1]
  return null
}
