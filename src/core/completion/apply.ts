...
function coversListEdit(listEdit: CompletionItem['textEdit'], resolvedEdit: CompletionItem['textEdit']): boolean {
  if (!listEdit || !resolvedEdit) return false
...
export function mergeResolved(item: CompletionItem, resolved?: CompletionItem): ItemWithMode {
  if (!resolved || resolved === item) return item
  const merged: ItemWithMode = { ...item }
  if (resolved.additionalTextEdits) merged.additionalTextEdits = resolved.additionalTextEdits
  if (resolved.command) merged.command = resolved.command
  ...
  if (resolved.textEdit && replaces) {
    ...
    if (resolved.insertTextFormat) merged.insertTextFormat = resolved.insertTextFormat
    if ((resolved as ItemWithMode).insertTextMode) merged.insertTextMode = (resolved as ItemWithMode).insertTextMode
  }
  if (item.insertText === undefined && resolved.insertText !== undefined) merged.insertText = resolved.insertText
  if (!item.insertTextFormat && resolved.insertTextFormat) merged.insertTextFormat = resolved.insertTextFormat
  return merged
}
...
function callIsUnsafe(doc: Text, from: number, to: number): boolean {
  if (doc.sliceString(to, to + 1) === '(') return true
...
function wantsCall(item: CompletionItem, raw: string, doc: Text, from: number, to: number): boolean {
  if (!item.kind || ![2, 3, 4].includes(item.kind)) return false
  if (item.textEdit || item.insertTextFormat === 2 || raw.includes('(')) return false
...
export function planCompletion(input: PlanInput): CompletionPlan {
  ...
  const toOffset = (pos: Position, assoc: -1 | 1) => {
  ...
  if (range) {
    ...
    if (start > head) warnings.push('textEdit range starts after the cursor — using the word range')
  }
  if (!usedRange && mode === 'replace') to = Math.max(to, wordEnd(doc, Math.max(to, head)))
  from = Math.min(from, head)
  to = Math.max(to, head)
  ...
  if (!isSnippet && indent) insert = raw.replace(/\n/g, `\n${indent}`)
  if (!isSnippet) cursor = insert.length
  ...
  // Additional edits, on the original document.
  ...
  const tagged = [main, ...extras].map((change, order) => ({ change, order }))
  tagged.sort((a, b) => {
    if (a.change.from !== b.change.from) return a.change.from - b.change.from
    const aEmpty = a.change.to === a.change.from
    const bEmpty = b.change.to === b.change.from
    if (aEmpty !== bEmpty) return aEmpty ? -1 : 1
    return a.order - b.order
  })
  const changes = tagged.map((t) => t.change)

  let shift = 0
  for (const change of changes) {
    if (change === main) break
    shift += change.insert.length - (change.to - change.from)
  }
  const start = main.from + shift
...
function overlaps(a: PlannedChange, b: PlannedChange): boolean {
  ...
  if (aEmpty) return a.from > b.from && a.from < b.to
  if (bEmpty) return b.from > a.from && b.from < a.to
...
function touches(from: number, to: number, span: { from: number; to: number }): boolean {
  if (span.from === span.to) return span.from > from && span.from < to
