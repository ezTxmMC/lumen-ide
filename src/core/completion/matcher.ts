...
function isAlnum(ch: string, code: number): boolean {
  if (code < 128) return isDigit(code) || (code >= 65 && code <= 90) || (code >= 97 && code <= 122)
  return WORD_CHAR.test(ch)
}

/** Character → bit of the character set (a–z each their own, the rest pooled). */
function charBit(lower: number): number {
  if (lower >= 97 && lower <= 122) return 1 << (lower - 97)
  if (isDigit(lower)) return 1 << 26
  if (lower === 95) return 1 << 27
  if (lower === 36) return 1 << 28
  return 1 << (29 + (lower % 3))
}
...
export function rawScore(q: Query, c: Prepared): number {
  ...
  if (m < n - maxErr) return NO_MATCH
  if (popcount(q.mask & ~c.mask) > maxErr) return NO_MATCH
  ...
  if (i === n) return finish(q, c, m, align(q, c, m, 0))
  if (maxErr === 0) return NO_MATCH
  ...
  for (let k = 0; k < n; k++) {
    ...
    for (let j = from; j < m; j++) {
      if (wl[j] !== pl[k]) continue
      found = j
      break
    }
    if (found < 0) {
      errors++
      if (errors > maxErr + 1) return NO_MATCH
      continue
    }
    from = found + 1
  }
...
function finish(q: Query, c: Prepared, m: number, value: number): number {
  if (value <= NO_MATCH) return NO_MATCH
  if (lastErrors > 0) return value
  const n = q.n
  const wl = c.lower
  for (let k = 0; k < n; k++) {
    if (wl[k] !== q.lower[k]) return value
  }
  let bonus = PREFIX_ALL
  if (c.text.startsWith(q.text)) bonus += PREFIX_CASE
  if (m === n && c.text.length === n) bonus += EXACT
  return value + bonus
}
...
function align(q: Query, c: Prepared, m: number, maxErr: number): number {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    const errPenalty = i === 1 ? FIRST_ERROR : 0
    const transAllowed = maxErr > 0 && i >= 2 && (i > 2 || firstError) && pc !== pl[i - 2]
    const transPenalty = i === 2 ? FIRST_ERROR : 0

    // Column 0: only a surplus query character is possible.
    A[row] = NEG
    N[row] = NEG
    ...
    for (let j = 1; j <= m; j++) {
      ...
      if (wc === pc) {
        ...
        if (j === 1 && i === 1) s += PREFIX
        if (pUpper && (f & FLAG_UPPER)) s += CASE_UPPER
        if (pUpper && !(f & FLAG_UPPER)) s -= CASE_MISS
        const fromA = A[diag] > LIMIT ? A[diag] + CONTIG : NEG
        let fromN = N[diag]
        // Entering mid-word, after a gap, costs extra.
        if (fromN > LIMIT && bound === 0 && j > 1) fromN -= i === 1 ? MID_START : GAP_MID
  ...
  if (N[end] <= LIMIT) return NO_MATCH
  lastErrors = NE[end]
  return N[end]
}
...
export function matchRanges(q: Query, c: Prepared): number[] {
  if (rawScore(q, c) <= NO_MATCH) return []
  const n = q.n
  ...
  while (i > 0) {
    ...
    const from = AT[cell]
    if (from === 1 || from === 2) positions.push(j - 1)
    if (from === 6 || from === 7) positions.push(j - 1, j - 2)
    if (from === 0) break
    if (from === 5) {
      i--
      continue
