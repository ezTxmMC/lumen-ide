...
class SnippetReader {
  ...
  private braced(node: SnippetNode): SnippetNode | null {
    ...
    if (c === '/') return this.transform() ? node : null
    return null
  }
...
function findDefs(nodes: SnippetNode[], defs: RenderContext['defs']) {
  for (const node of nodes) {
    ...
    if (node.t === 'stop' && hasDefault(node) && !defs.has(node.n)) defs.set(node.n, node)
...
function renderStop(node: SnippetNode & { t: 'stop' }, ctx: RenderContext, record: boolean) {
  ...
  if (source && !ctx.active.has(node.n)) {
    ...
    if (source.choices?.length) emit(ctx, source.choices[0])
    if (!source.choices?.length) render(source.kids, ctx, record && primary)
  ...
  if (node.choices?.length && !ctx.choices.has(node.n)) ctx.choices.set(node.n, node.choices)
}
