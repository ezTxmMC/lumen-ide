...
export const debugpy: DebugAdapterConfig = {
  ...
  launch: async (ctx) => {
    if (!ctx.file) {
      throw new Error(t('debug.error.noFile'));
    }
...
function jsDebug(typescript: boolean): DebugAdapterConfig {
  return {
    ...
    launch: async (ctx) => {
      ...
      if (await ctx.exists(tsx)) {
        return { ...base, runtimeExecutable: tsx };
      }
...
export const javaDebug: DebugAdapterConfig = {
  ...
  connect: async (ctx) => {
    ...
    if (typeof port !== 'number') {
      throw new Error(t('debug.java.noPort'));
    }
    return { port };
  },
  launch: async (ctx) => {
    ...
    if (!options.length) {
      throw new Error(t('debug.java.noMainClass'));
    }
