...
function markClasses(bp: BreakpointEntry) {
  ...
  const classes = ['lm-bp']
  if (!bp.enabled) classes.push('lm-bp-disabled')
  if (bp.logMessage) classes.push('lm-bp-log')
  if (bp.condition || bp.hitCondition) classes.push('lm-bp-conditional')
  if (bp.enabled && debug.hasSessions && status && !status.verified) classes.push('lm-bp-unverified')
  return classes.join(' ')
}
...
function markTitle(bp: BreakpointEntry) {
  ...
  if (bp.condition) parts.push(t('debug.bp.conditionIs', { condition: bp.condition }))
  if (bp.hitCondition) parts.push(t('debug.bp.hitIs', { hits: bp.hitCondition }))
  if (!bp.enabled) parts.push(t('debug.bp.disabled'))
  if (status && !status.verified) parts.push(status.message ?? t('debug.bp.unverified'))
  return parts.join('\n')
}
...
function execFor(state: EditorState, path: string | null): ExecMark | null {
  ...
  if (!exec || !samePath(exec.path, path)) return null
...
const execField = StateField.define<ExecMark | null>({
  ...
  update(value, tr) {
    for (const effect of tr.effects) if (effect.is(setExec)) return effect.value
  ...
  provide: (field) => EditorView.decorations.from(field, (exec) => {
    if (!exec) return Decoration.none
...
function inlineFor(path: string | null): InlineInfo | null {
  ...
  if (!inline || !debug.showInline || !samePath(inline.path, path)) return null
...
const inlineField = StateField.define<InlineInfo | null>({
  ...
  update(value, tr) {
    for (const effect of tr.effects) if (effect.is(setInline)) return effect.value
...
function inlineDecorations(state: EditorState): DecorationSet {
  ...
  if (!info || !info.values.size) return Decoration.none
  ...
  for (let number = last; number >= first; number--) {
    for (const match of state.doc.line(number).text.matchAll(IDENTIFIER)) {
      const name = match[0]
      if (seen.has(name) || !info.values.has(name)) continue
      seen.add(name)
...
class BreakpointMarker extends GutterMarker {
  ...
  toDOM() {
    ...
    if (this.title) wrap.title = this.title
    if (this.classes) {
...
function gutterMarkers(view: EditorView) {
  ...
  for (const mark of marks) byPos.set(mark.pos, { classes: mark.classes, title: mark.title, exec: null })
  if (exec) {
...
function expressionAt(state: EditorState, pos: number): { from: number; to: number; text: string } | null {
  ...
  if (start === end || /^\d/.test(text.slice(start, end))) return null
  for (;;) {
    if (start > 0 && text[start - 1] === '.') {
      let before = start - 1
      while (before > 0 && WORD.test(text[before - 1])) before--
      if (before === start - 1) break
      start = before
      continue
    }
...
const debugHover = hoverTooltip(async (view, pos): Promise<Tooltip | null> => {
  if (!debug.isStopped) return null
...
function syncView(view: EditorView) {
  ...
  if (currentMarks && !pendingPush.has(view) && !sameMarks(marks, currentMarks) && inSync(view)) effects.push(setMarks.of(marks))
  const exec = execFor(view.state, path)
  const current = view.state.field(execField)
  if (exec?.pos !== current?.pos || exec?.top !== current?.top) effects.push(setExec.of(exec))
  const inline = inlineFor(path)
  const currentInline = view.state.field(inlineField)
  if (inline?.values !== currentInline?.values || inline?.line !== currentInline?.line) effects.push(setInline.of(inline))
...
function syncAll() {
  ...
  queueMicrotask(() => {
    syncQueued = false
    for (const view of liveViews) syncView(view)
  })
}
...
function pushLines(view: EditorView) {
  if (pendingPush.has(view)) return
  ...
  queueMicrotask(() => {
    ...
    for (const mark of view.state.field(marksField)) lines.set(mark.id, view.state.doc.lineAt(mark.pos).number - 1)
...
const syncPlugin = ViewPlugin.fromClass(class {
  constructor(readonly view: EditorView) {
    ...
    queueMicrotask(() => {
      if (liveViews.has(view)) syncView(view)
    })
  }

  update(update: ViewUpdate) {
    if (update.docChanged && update.state.field(marksField, false)?.length) pushLines(update.view)
  }
...
export function debugEditorExtension(ctx: EditorContext): Extension {
  ...
  if (!ctx.path || /^[a-z][\w+.-]*:\/\//i.test(ctx.path)) return base
