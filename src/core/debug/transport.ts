...
function wire() {
  ...
  window.lumen.dap.onMessage(({ id, message }) => {
    for (const cb of routes.get(id)?.message ?? []) {
      cb(message as unknown as DapMessage);
    }
  });
  window.lumen.dap.onOutput(({ id, stream, text }) => {
    for (const cb of routes.get(id)?.output ?? []) {
      cb(stream, text);
    }
  });
  ...
  return {
    ...
    send: (message) => {
      void window.lumen.dap.send(id, message).then((ok) => {
        ...
        routes.delete(id);
        for (const cb of current?.close ?? []) {
          cb('Adapter nicht erreichbar');
        }
      });
    },
    onMessage: (cb) => {
