...
class Debugger {
  ...
  private flushPersist() {
    ...
    if (!this.stateFileExists && isEmptyState(this.state)) {
      return;
    }
  ...
  private readonly memory: DebugContext['memory'] = {
    ...
    set: (key, value) => {
      if (this.state.memory[key] === value) {
        return;
      }
  ...
  private allFileBreakpoints(): FileBreakpoints[] {
    ...
    for (const bp of breakpoints.all()) {
      paths.set(normalizePath(bp.path), bp.path);
    }
    if (this.tempBreakpoint) {
      paths.set(normalizePath(this.tempBreakpoint.path), this.tempBreakpoint.path);
    }
  ...
  private scheduleResend(path: string) {
    ...
    this.resendTimers.set(key, setTimeout(() => {
      ...
      for (const session of this.sessions) {
        if (session.state === 'initializing' || session.state === 'terminated') {
          continue;
        }
  ...
  private readonly hooks: SessionHooks = {
    ...
    startDebugging: async (session, args) => {
      ...
      if (!meta) {
        throw new Error('startDebugging');
      }
  ...
  private handleTerminated(session: DebugSession, reason: string) {
    ...
    if (session.parent) {
      session.parent.children = session.parent.children.filter((c) => c !== session);
    }
    ...
    if (session.exitCode !== null) {
      this.append('system', t('debug.console.exited', { name: session.name, code: session.exitCode }), session.id);
    }
    if (this.focus?.sessionId === session.id) {
      this.focus = null;
      this.setExec(null);
      ...
      if (next) {
        void this.handleStopped(next, [...next.threads.values()].find((th) => th.stopped)?.id, false, 'focus');
      }
    }
    if (!this.sessions.length) {
      this.allEnded();
  ...
  private syncActive() {
    ...
    if (useStore.getState().debugActive !== active) {
      useStore.setState({ debugActive: active });
    }
  }
  ...
  private resolveProgram(adapter: DebugAdapterConfig, vars: Vars) {
    ...
    if (!hit) {
      ...
      void hit.then((result) => { if (!result) {
  ...
  async start(options: { pick?: boolean; choiceId?: string; } = {}) {
    ...
    try {
      ...
      for (const choice of choices) {
        if (choice.config || await this.isAvailable(choice, vars)) {
  ...
  private async startSession(
    ...
    if (parent) {
      parent.children = [...parent.children, session];
    }
  ...
  focusedThread(): ThreadState | null {
    ...
    if (!session || this.focus?.threadId === null || this.focus?.threadId === undefined) {
      return null;
    }
  ...
  private target(): { session: DebugSession; threadId: number; } | null {
    ...
    if (session && threadId !== null && threadId !== undefined) {
    ...
    for (const candidate of this.sessions) {
      ...
      if (thread) {
        return { session: candidate, threadId: thread.id };
      }
    }
    return null;
  }
  ...
  async debugFrom(path: string, line: number) {
    if (!breakpoints.at(path, line)) {
      breakpoints.add(path, line);
    }
    if (this.sessions.length) {
      return;
    }
  ...
  async selectThread(sessionId: string, threadId: number) {
    ...
    if (thread.stopped && !thread.frames.length) {
      await session.loadFrames(threadId).catch(() => {});
    }
  ...
  private async computeInline() {
    ...
    if (!session || !frame || !this.exec || !this.showInline) {
      return;
    }
    ...
    for (const scope of cheap) {
      ...
      for (const variable of variables.slice(0, 200)) {
        if (!/^[A-Za-z_$][\w$]*$/.test(variable.name) || values.has(variable.name)) {
          continue;
        }
        values.set(variable.name, variable.value);
      }
    }
    if (this.focus?.frameId !== frame.id) {
      return;
    }
  ...
  setShowInline(show: boolean) {
    ...
    if (show) {
      void this.computeInline().then(() => this.emit());
    }
    this.emit();
  }
  ...
  toggleExceptionFilter(type: string, filter: string) {
    ...
    this.schedulePersist();
    for (const session of this.sessions) {
      if (session.type !== type || session.state === 'terminated') {
        continue;
      }
