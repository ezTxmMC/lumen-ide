...
export function allAdapters(): { adapter: DebugAdapterConfig; language: LanguageSpec; }[] {
  ...
  for (const language of registry.languages()) {
    for (const adapter of language.debug ?? []) {
      out.push({ adapter, language });
    }
  }
  return out;
}
...
export async function openLaunchConfigFile() {
  ...
  if (!exists) {
    await window.lumen.fs.writeFile(file, `${JSON.stringify(TEMPLATE, null, 2)}\n`);
  }
...
export function substituteDeep<T>(value: T, vars: Vars): T {
  if (typeof value === 'string') {
    return substitute(value, vars) as T;
  }
...
async function readyClient(file: string | null, languageId: string | null) {
  ...
  while (Date.now() - started < 180_000) {
    ...
    if (client && client.status !== 'starting' && client.status !== 'idle' && client.status !== 'checking') {
      return null;
    }
    if (!client && !opened && file) {
      ...
      if (tab) {
        void lsp.openDocument(state.languageFor(tab), file, tab.content);
      }
    }
    if (!client && Date.now() - started > 10_000) {
      return null;
    }
    await waitMs(400);
  }
...
async function restartForBundles(clientId: string) {
  ...
  for (const path of files) {
    ...
    if (tab) {
      await lsp.openDocument(state.languageFor(tab), path, tab.content);
    }
  }
}
...
async function lspCommand(file: string | null, languageId: string | null, command: string, args: unknown[] = [], retried = false): Promise<unknown> {
  ...
  if (!client) {
    throw new Error(t('debug.error.noLsp'));
  }
  try {
    ...
    if (!missingHandler || retried || bundlesAddedAt <= client.startedAt) {
      throw err;
    }
...
export async function injectLspBundles() {
  for (const language of registry.languages()) {
    ...
    for (const config of language.lsp ?? []) {
      ...
      if (!options || !Array.isArray(options.bundles)) {
        continue;
      }
      for (const jar of jars) {
        if (!options.bundles.includes(jar)) {
          options.bundles.push(jar);
        }
      }
