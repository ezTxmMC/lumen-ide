...
export class DebugSession {
  ...
  async start(): Promise<void> {
    ...
    if (launchDone) {
      await Promise.race([initialized, delay(5000)]);
    }
  ...
  async setBreakpoints(file: FileBreakpoints) {
    ...
    list.forEach((bp, i) => {
      if (bp.id !== undefined && file.ids[i]) {
        this.breakpointIds.set(bp.id, file.ids[i]);
      }
    });
  ...
  private async handleReverseRequest(command: string, args: unknown): Promise<unknown> {
    if (command === 'runInTerminal') {
      return this.hooks.runInTerminal(this, args as RunInTerminalArguments);
    }
  ...
  private async handleStopped(body: StoppedEventBody) {
    ...
    for (const thread of this.threads.values()) {
      if (!all && thread.id !== body.threadId) {
        continue;
      }
      thread.stopped = true;
    ...
    if (threadId !== undefined) {
      await this.loadFrames(threadId).catch(() => {});
    }
  ...
  private markRunning(threadId?: number) {
    ...
    this.variableCache.clear();
    this.scopeCache.clear();
    if (![...this.threads.values()].some((t) => t.stopped) && this.state !== 'terminated') {
      this.state = 'running';
    }
  ...
  async stop() {
    ...
    if (this.capabilities.supportsTerminateRequest && this.request === 'launch') {
      ...
      if (ok) {
        await Promise.race([this.waitForEnd(), delay(2500)]);
      }
    }
    await this.shutdown();
