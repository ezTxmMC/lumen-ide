...
export function templateCategory(
  ...
  if (template.category?.trim()) {
    return { id: `cat:${template.category.trim()}`, label: translate(template.category.trim()) };
  }
  const language = languageOf(template, languages);
  if (language) {
    return { id: `lang:${language.id}`, label: language.name };
  }
...
export function filterTemplates(
  ...
  const inCategory = (template: ProjectTemplate) => {
    ...
    if (query.category === RECENT_CATEGORY) {
      return recent.includes(template.id);
    }
  ...
  const score = (template: ProjectTemplate) => {
    ...
    if (words.every((word) => name.startsWith(word) || name.includes(` ${word}`))) {
      return 2;
    }
