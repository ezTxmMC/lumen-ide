...
export function fieldChoices(field: FormField, values: FormValues, loaded: LoadedChoices = {}): FieldChoice[] {
  ...
  if (state && state.choices.length) return state.choices
  if (field.choicesFor) return safeChoicesFor(field, values)
...
export function choicesSettled(field: FormField, values: FormValues, loaded: LoadedChoices = {}): boolean {
  if (!field.loadChoices) return true
...
export class ChoiceLoader {
  ...
  sync(fields: FormField[], values: FormValues) {
    ...
    for (const field of fields) {
      if (!field.loadChoices) continue
      if (field.when && !safeWhen(field, values)) continue
      ...
      if (this.states[field.id]?.key === key && this.started[field.id] === attempt) continue
  ...
  private pending(field: FormField | undefined, values: FormValues): boolean {
    if (!field?.loadChoices) return false
    if (field.when && !safeWhen(field, values)) return false
  ...
  private hold(id: string) {
    const state = this.states[id]
    if (state?.status === 'loading' && !state.key) return
  ...
  private start(field: FormField, values: FormValues, key: string) {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (this.controllers.get(field.id) === controller) this.controllers.delete(field.id)
      }
    }
    const promise = run()
