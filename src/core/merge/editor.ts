...
function hasStartMarker(doc: Text): boolean {
  for (const iter = doc.iterLines(); !iter.next().done;) {
    if (iter.value.startsWith('<<<<<<<')) return true
  }
  return false
}
...
class ConflictLens extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    LENS.forEach(({ action, label }, index) => {
      if (index > 0) row.append(Object.assign(document.createElement('span'), { className: 'lm-merge-lens-sep', textContent: '|' }))
...
function buildDecorations(state: EditorState): DecorationSet {
  ...
  conflicts.forEach((conflict, index) => {
    if (!readonly) out.push(Decoration.widget({ widget: new ConflictLens(index), block: true, side: -1 }).range(conflict.from))
