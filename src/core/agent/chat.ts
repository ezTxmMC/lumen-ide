...
function absoluteIn(root: string | null, file: string) {
  if (!root || /^([\\/]|[A-Za-z]:[\\/])/.test(file)) return file
...
function todosFromTool(name: string, input: Record<string, unknown>): TodoEntry[] | null {
  if (name !== 'TodoWrite' || !Array.isArray(input.todos)) return null
  return input.todos
...
const HANDLERS: { [K in AgentEvent['kind']]: (chat: Chat, event: Extract<AgentEvent, { kind: K }>) => void } = {
  session(chat, event) {
    ...
    if (event.slashCommands?.length) chat.slashCommands = event.slashCommands
  },
  assistant(chat, event) {
    settleStreams(chat, true)
    for (const block of event.blocks) {
      if (block.type === 'text') push(chat, { role: 'assistant', text: block.text })
      if (block.type === 'thinking') push(chat, { role: 'thinking', text: block.text })
  ...
  toolResult(chat, event) {
    ...
    if (tool?.role !== 'tool' || event.isError) return
    ...
    if (!FILE_TOOLS.has(tool.name)) return
  ...
  result(chat, event) {
    ...
    if (event.isError && event.text) push(chat, { role: 'note', text: event.text, error: true })
...
function editorContext(root: string): { prompt: string; label?: string } {
  ...
  if (!tab?.path || tab.virtual) return { prompt: '' }
  ...
  if (!selection) return { prompt: `[The user has ${file} open, cursor at line ${line}.]`, label: `${file}:${line}` }
...
export const agentChat = {
  ...
  async loadModels(key: string, refresh = false) {
    ...
    if (!info || state.modelsLoading) return
    if (state.reportedModels && !refresh) return
  ...
  effort(key: string): string {
    ...
    if (!effort || !efforts.some((entry) => entry.id === effort)) return ''
    return effort
  },
  ...
  setEffort(key: string, effort: string) {
    ...
    if (effort) efforts[agentChat.model(key)] = effort
  ...
  closeChat(key: string, chatId: string) {
    ...
    if (state.activeChatId === chatId) state.activeChatId = state.chats[state.chats.length - 1].id
    emit()
  },
  ...
  async send(key: string, text: string, attachments: DraftAttachment[] = []) {
    ...
    if (!chat.items.some((item) => item.role === 'user')) chat.title = titleFrom(message)
  ...
  async answer(key: string, requestId: string, allow: boolean, remember = false, message?: string) {
    ...
    if (chat) patch(chat, isRequest, (item) => ({ ...item, state: allow ? 'allowed' : 'denied' }) as ChatItem)
    emit()
