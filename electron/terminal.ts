...
function loadPty(): PtyModule {
  ...
  try {
    ...
    ptyError = `node-pty konnte nicht geladen werden (${(err as Error).message.split('\n')[0]}). ` +
      'Mit „npm run rebuild:native“ für Electron neu bauen.'
...
export function detectShells(): ShellProfile[] {
  if (process.platform === 'win32') return windowsShells()
  ...
  for (const file of [userShell, ...candidates]) {
    ...
    if (NOT_INTERACTIVE.has(name) || byName.has(name)) continue
    byName.set(name, {
      id: name,
  ...
  if (!shells.some((s) => s.isDefault) && shells[0]) shells[0].isDefault = true
  return shells
}
...
function loginArgs(name: string): string[] {
  if (['bash', 'zsh', 'sh', 'ksh', 'dash'].includes(name)) return ['-l']
  if (name === 'fish') return ['--login']
  if (name === 'pwsh') return ['-NoLogo']
  return []
}
...
function windowsShells(): ShellProfile[] {
  ...
  if (shells[0]) shells[0].isDefault = true
  return shells
}
...
export function detectExternalTerminals(): ExternalTerminal[] {
  const found = terminalSpecs().filter((t) => {
    if (process.platform === 'darwin') return darwinAppExists(t.label)
    if (process.platform === 'win32') return t.id === 'cmd' || Boolean(which(t.command))
...
export function openExternalTerminal(cwd: string, terminalId?: string): string {
  ...
  if (!spec) throw new Error('No terminal program found')
...
function cleanEnv(env: NodeJS.ProcessEnv): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(env)) {
    ...
    if (key.startsWith('ELECTRON_') || key === 'VITE_DEV_SERVER_URL' || key === 'APP_ROOT') continue
    out[key] = value
  }
  return out
}
...
export function createTerminal(id: string, options: TerminalOptions, contents: WebContents) {
  ...
  instance.onData((data) => {
    ...
    session.timer = setTimeout(() => {
      ...
      session.buffer = ''
      if (!contents.isDestroyed()) contents.send('terminal:data', { id, data: chunk })
    }, 6)
  })
