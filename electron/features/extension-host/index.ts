...
async function readVerified(id: string, hash: string): Promise<CodeParts | null> {
  ...
  if (main === undefined && renderer === undefined) {
    return null;
  }
  if (sha256(codeHashInput({ main, renderer })) !== hash) {
    return null;
  }
...
async function activate(id: string, file: string) {
  ...
  if (typeof mod.activate !== 'function') {
    throw new Error(`${id} has no activate() function`);
  }
...
async function installCode(id: string, raw: CodeParts | string, hash: string) {
  ...
  if (!code.main && !code.renderer) {
    throw new Error('No code given');
  }
  if (Buffer.byteLength(`${code.main ?? ''}${code.renderer ?? ''}`) > MAX_CODE_BYTES) {
    throw new Error('The code is too large');
  }
...
async function pruneCode(id: string, keep: string[] = []) {
  ...
  for (const name of names) {
    if (!name.startsWith(`${id}.`) || !name.endsWith('.mjs')) {
      continue;
    }
...
async function activateRecord(name: string) {
  ...
  if (!id || !hash || !EXTENSION_ID.test(id) || !HASH.test(hash)) {
    return;
  }
...
async function rendererCode(id: string): Promise<string | null> {
  ...
  if (!record.codeHash || !HASH.test(record.codeHash)) {
    return null;
  }
