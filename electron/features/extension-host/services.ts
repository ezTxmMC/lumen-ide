...
function encrypt(value: string): string {
  if (!safeStorage.isEncryptionAvailable()) throw new Error('No system key store available to keep secrets safely')
...
export const services = {
  ...
  setSettings(extensionId: string, values: Record<string, string>) {
    ...
    for (const fn of listenersOf(settingListeners, extensionId)) fn(clean)
  },
  ...
  async getSecret(extensionId: string, key: string): Promise<string | undefined> {
    if (!SECRET_KEY.test(key)) throw new Error(`Invalid secret key: ${key}`)
  ...
  async setSecret(extensionId: string, key: string, value: string) {
    ...
    delete data[key]
    if (typeof value === 'string' && value) data[key] = encrypt(value)
    await writeJson(file, data)
    for (const fn of listenersOf(settingListeners, extensionId)) fn(settings.get(extensionId) ?? {})
  },
  ...
  request<T>(extensionId: string, request: UiRequest): Promise<T | null> {
    ...
    return new Promise((resolve) => {
      ...
      if (!send('extensions:ui:request', { requestId, extensionId, ...request })) services.answer(requestId, null)
    })
  },
  ...
  dispatch(event: HostEvent) {
    ...
    if (event.kind === 'locale' && typeof event.language === 'string') locale = event.language
  ...
  removeExtension(extensionId: string) {
    for (const [requestId, entry] of pending) {
      if (entry.extensionId === extensionId) services.answer(requestId, null)
    }
