...
export function exec(command: string, args: string[] = [], options: ExecOptions = {}): Promise<ExecResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout?.on('data', (chunk: Buffer) => { if (stdout.length < MAX_OUTPUT) {
      stdout += chunk.toString();
    } });
    child.stderr?.on('data', (chunk: Buffer) => { if (stderr.length < MAX_OUTPUT) {
...
type Track = (dispose: Dispose) => Dispose;
type Send = (body: UiMessage) => ReturnType<typeof services.message>;

function viewsApi(extensionId: string, message: Send) {
  return {
...
function eventsApi(extensionId: string, track: Track) {
  return {
...
function uiApi(extensionId: string, message: Send) {
  return {
...
function settingsApi(extensionId: string, track: Track) {
  return {
...
function secretsApi(extensionId: string) {
  return {
...
function storageApi(extensionId: string) {
  return {
...
export function createContext(extensionId: string, onDispose: (fn: Dispose) => void) {
  ...
  const track: Track = (dispose) => {
    onDispose(dispose);
    return dispose;
  };
  const message: Send = (body) => services.message(extensionId, body);
  ...
  return {
    ...
    views: viewsApi(extensionId, message),
    ...
    events: eventsApi(extensionId, track),

    ui: uiApi(extensionId, message),

    exec,

    settings: settingsApi(extensionId, track),

    secrets: secretsApi(extensionId),
