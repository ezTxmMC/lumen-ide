...
function checkLocalId(id: string, what: string) {
  if (typeof id !== 'string' || !LOCAL_ID.test(id)) throw new Error(`Invalid ${what} id: ${id}`)
}
...
function countNodes(content: ViewContent): { nodes: number; rows: number } {
  ...
  const walk = (nodes: ViewContent['nodes']) => {
    for (const node of nodes) {
      count++
      if (node.type === 'section' || node.type === 'row') walk(node.children)
      if (node.type === 'item' && node.children) walk(node.children)
      if (node.type === 'grid') rows += node.rows?.length ?? 0
    }
  }
  walk(content.nodes ?? [])
...
export const contributions = {
  ...
  setStatus(extensionId: string, itemId: string, item: StatusItem | null) {
    ...
    if (!item) status.delete(key)
    if (item) status.set(key, { ...item, text: String(item.text ?? '').slice(0, 120), extensionId, id: itemId })
  ...
  async render(extensionId: string, viewId: string, instance?: string): Promise<ViewContent> {
    ...
    if (!provider) return { nodes: [] }
    const content = await provider.render(typeof instance === 'string' ? instance : undefined)
    if (!content || !Array.isArray(content.nodes)) return { nodes: [] }
    const size = countNodes(content)
    if (size.nodes > MAX_NODES) throw new Error(`The view ${viewId} is too large (more than ${MAX_NODES} entries)`)
    if (size.rows > MAX_GRID_ROWS) throw new Error(`The view ${viewId} is too large (more than ${MAX_GRID_ROWS} rows)`)
    return content
  },

  async action(extensionId: string, viewId: string, event: ViewActionEvent) {
    const provider = views.get(keyOf(extensionId, viewId))
    if (!provider) throw new Error(`View not available: ${viewId}`)
    ...
    if (event?.action === '__close') {
      if (instance !== undefined) provider.onClose?.(instance)
      return null
    }
    if (!provider.onAction) return null
  ...
  async runCommand(extensionId: string, commandId: string, args?: unknown) {
    ...
    if (!handler) throw new Error(`Command not available: ${commandId}`)
    return handler(args)
  },
