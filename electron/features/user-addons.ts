...
function addonFile(id: string): string {
  ...
  if (rel.startsWith('..') || path.isAbsolute(rel) || rel.includes(path.sep)) {
    throw new Error('Path outside the add-on folder');
  }
  return file;
}
...
async function saveAddon(id: string, content: string) {
  if (typeof content !== 'string' || content.length > MAX_ADDON_BYTES) {
    throw new Error('Contents invalid or too large');
  }
  JSON.parse(content);
...
function execShell(command: string, cwd: string | null): Promise<ShellExecResult> {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      stderr += `\nTimed out after ${SHELL_TIMEOUT_MS / 1000} s`;
      child.kill('SIGTERM');
      finish(-1);
    }, SHELL_TIMEOUT_MS);
