...
export function splitPackages(text: string): string[] {
  ...
  if (!names.length) throw new Error('No package given')
...
export function installInvocation(manager: PackageManagerId, packages: string[]): InstallInvocation {
  ...
  if (!packages.length) throw new Error('No package given')
  for (const name of packages) {
    if (!isValidPackageName(name)) throw new Error(`Invalid package name: ${name}`)
  }
  if (spec.single && packages.length > 1) throw new Error(`${spec.label} installs one package at a time`)
...
export function candidateManagers(platform: string, preferred: PackageManagerId | null): PackageManagerId[] {
  ...
  if (!preferred || !own.includes(preferred)) return own
...
export function commandNeedsRoot(command: string): boolean {
  ...
  if (ELEVATORS.test(text)) return true
