...
function plan(request: PrivilegedRequest): InstallInvocation {
  if (request.kind === 'packages') {
    if (!isPackageManager(request.manager)) throw new Error(`Unknown package manager: ${request.manager}`)
    if (!Array.isArray(request.packages)) throw new Error('No package given')
    return installInvocation(request.manager, request.packages.map(String))
  }
  if (request.kind !== 'command') throw new Error('Unknown request')
  if (request.confirmed !== true) throw new Error('The command has not been confirmed')
...
function withEnv(invocation: InstallInvocation): string[] {
  ...
  if (!pairs.length) return invocation.argv
...
async function launchFor(invocation: InstallInvocation, request: PrivilegedRequest, info: SystemInfo): Promise<Launch | 'needsPassword'> {
  ...
  if (!invocation.root || info.isRoot || IS_WINDOWS) return { argv: [...invocation.argv], display }
  const elevated = formatInvocation(invocation, true)
  if (request.method === 'pkexec' && info.pkexec) return { argv: ['pkexec', ...withEnv(invocation)], display: `pkexec ${display}` }
  if (!info.sudo) throw new Error('Root rights are needed, but neither sudo nor pkexec is installed')
  if (typeof request.password === 'string' && request.password) {
    return { argv: ['sudo', '-S', '-k', '-p', '', ...withEnv(invocation)], stdin: `${request.password}\n`, display: elevated }
  }
  if (await sudoWithoutPassword()) return { argv: ['sudo', '-n', ...withEnv(invocation)], display: elevated }
...
async function run(jobId: string, request: PrivilegedRequest, getWindow: () => BrowserWindow | null): Promise<PrivilegedResult> {
  ...
  if (launch === 'needsPassword') return { ok: false, code: null, needsPassword: true }
