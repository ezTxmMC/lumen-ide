...
async function isGeneratedBin(dir: string): Promise<boolean> {
  ...
  if (!build.some(Boolean)) return false
  let classes = 0
...
async function moduleMetadata(dir: string): Promise<string[]> {
  ...
  if (generated) found.push(path.join(dir, '.project'))
  for (const name of ['.classpath', '.factorypath']) {
    const content = await fs.readFile(path.join(dir, name), 'utf8').catch(() => null)
    if (content === null) continue
    if (generated || GENERATED_CLASSPATH.test(content)) found.push(path.join(dir, name))
  }
  const settings = path.join(dir, '.settings')
  if (generated ? await isGeneratedSettings(settings) : await isGeneratedPrefsOnly(settings)) found.push(settings)
  if (await isGeneratedLaunches(path.join(dir, '.eclipse'))) found.push(path.join(dir, '.eclipse'))
  if (await isGeneratedBin(dir)) found.push(path.join(dir, 'bin'))
  return found
}
