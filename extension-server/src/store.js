...
export class Store {
  ...
  async remove(id, version) {
    ...
    const rest = (await fs.readdir(dir).catch(() => []))
      .filter((name) => name.endsWith('.json') && VERSION_PATTERN.test(name.slice(0, -5)))
    if (!rest.length) await fs.rm(dir, { recursive: true, force: true })
    await this.reload(id)
    await this.writeIndex()
  }
  ...
  async manifest(id, version) {
    ...
    if (!version || version === 'latest') return entry.manifest
    if (!entry.meta.versions.includes(version)) return null
  ...
  search(query) {
    ...
    if (!needle) return all.sort((a, b) => (a.name.toLowerCase() < b.name.toLowerCase() ? -1 : 1))
