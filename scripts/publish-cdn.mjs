...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--no-archive') options.archive = false
    ...
    if (arg === '--notes') options.notes = argv[++i]
    if (arg === '--help' || arg === '-h') {
...
function listFiles(dir, depth = 2) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (!entry.isDirectory() || depth <= 1) continue
    if (entry.name.endsWith('-unpacked') || entry.name.endsWith('.app') || entry.name === 'cdn' || /^mac(-\w+)?$/.test(entry.name)) continue
...
function sftpBatch({ releases, version, manifestFile, previous, archive }) {
  ...
  if (archive) lines.push(`-mkdir ${quote(`${REMOTE_ROOT}/${version}`)}`)
  for (const release of releases) {
    const targets = [`${REMOTE_ROOT}/latest/${release.platform}`]
    if (archive) targets.push(`${REMOTE_ROOT}/${version}/${release.platform}`)
    for (const target of targets) {
      lines.push(`-mkdir ${quote(target)}`)
      for (const file of release.local) lines.push(`put ${quote(file)} ${quote(`${target}/${path.basename(file)}`)}`)
    }
  }
...
async function main() {
  ...
  if (!fs.existsSync(dir)) throw new Error(`${dir} does not exist — build first (npm run dist:…)`)

  const unknown = (options.only ?? []).filter((platform) => !PLATFORMS[platform])
  if (unknown.length) throw new Error(`Unknown platform: ${unknown.join(', ')} (known: ${Object.keys(PLATFORMS).join(', ')})`)
  ...
  if (!releases.length) throw new Error(`No packages for version ${options.version} in ${dir}`)
  ...
  for (const release of releases) {
    ...
    for (const file of described) console.log(`    ${file.name}  ${(file.size / 1024 / 1024).toFixed(1)} MB`)
  }
