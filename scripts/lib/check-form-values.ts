...
function chainFields(calls: string[], versions: FieldChoice[], delays: { game?: number; loader?: number; api?: number; } = {}): FormField[] {
  return [
    {
...
/** Selects, computed choices, grouping and search. */
function checkPlainChoices(ok: Ok) {
...
/** Three fetched fields in a chain, and how dependents wait. */
async function checkChainedLoads(ok: Ok) {
  const calls: string[] = [];
  const versions = gameVersions();
  const chain = (delays: { game?: number; loader?: number; api?: number; } = {}) => chainFields(calls, versions, delays);
  const fields = chain();
...
/** Overtaken loads, errors and retry, hidden fields. */
async function checkRaceAndRetry(ok: Ok) {
...
/** Categories, search, recent templates and the file tree. */
function checkCatalogue(ok: Ok) {
  console.log('\nTemplate catalogue:');
  const languages = [
...
export async function checkFormValues(ok: Ok, base: string) {
  ...
  checkPlainChoices(ok);
  await checkChainedLoads(ok);
  await checkRaceAndRetry(ok);
  checkCatalogue(ok);
  await checkRemoteTemplate(ok, base);
}
