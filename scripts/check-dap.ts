...
const events: DapEvents = {
  message: (id, message) => {
    ...
    for (const cb of handlerFor(id).message) {
      cb(message as DapMessage);
    }
  },
  output: (_id, _stream, text) => { if (verbose) {
    process.stdout.write(`  [adapter] ${text}`);
  } },
  closed: (id, reason) => { for (const cb of handlerFor(id).close) {
    cb(reason);
  } },
};
...
async function runScenario(s: Scenario) {
  ...
  const waiters: (() => void)[] = [];
  const poke = () => { for (const w of waiters.splice(0)) {
    w();
  } };
  ...
  const hooks: SessionHooks = {
    ...
    breakpointsVerified: (_session, ids, result) => {
      ...
      const state = result[0]?.verified ? 'confirmed' : `pending${result[0]?.message ? ` – ${result[0].message}` : ''}`;
    ...
    terminated: (session) => { if (!session.parent) {
      terminated = true;
    } poke(); },
    runInTerminal: async () => ({}),
  ...
  try {
    ...
    const evaluated = await stoppedSession.evaluate(s.expression, top.id, 'watch').catch((err: Error) => ({ result: `Error ${err.message}`, variablesReference: 0 }));
