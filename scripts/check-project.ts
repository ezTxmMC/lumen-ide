...
;(globalThis as unknown as { DOMParser: unknown }).DOMParser = class {
  parseFromString(text: string) {
    ...
    for (const m of text.matchAll(re)) {
      ...
      if (m[5]) stack[stack.length - 1].appendText(m[5])
    }
...
if (languagesOnly.length) console.log(`  ·  languages only, no templates: ${languagesOnly.join(', ')}`)
...
function variants(template: ProjectTemplate): FormValues[] {
  ...
  for (const field of fields) {
    ...
    if (field.type === 'select' || field.type === 'combobox') {
      for (const choice of fieldChoices(field, defaults)) {
        ...
        if (!visibleFields(fields, values).some((f) => f.id === field.id)) continue
        out.push(touched)
      }
    }
    if (field.type === 'toggle') out.push({ [field.id]: defaults[field.id] === 'true' ? 'false' : 'true' })
  }
  ...
  if (fields.some((f) => f.id === 'packages')) {
    for (const pm of ['vcpkg', 'conan']) {
      for (const tests of ['gtest', 'catch2', 'plain', 'none']) out.push({ build: 'cmake', packages: pm, tests })
    }
    for (const build of ['meson', 'xmake', 'bazel', 'make']) out.push({ build, tests: 'gtest' }, { build, type: 'library' })
  }
  return out
}
...
for (const template of templates) {
  ...
  for (const touched of variants(template)) {
    ...
    if (result.open) ok(await lumen.fs.exists(result.open), `${label}: zu öffnende Datei ${path.relative(result.dir, result.open)} existiert`, true)
    for (const task of result.setup) ok(Boolean(task.command) && Array.isArray(task.args), `${label}: Setup-Aufgabe ${task.id} vollständig`, true)
    ...
    if (info.primary) detectedKinds.add(info.primary.kind.id)
    ...
    if (!expected) ok(!info.primary || info.primary.kind.id !== 'npm' || template.id === 'html-site', `${label}: keine unerwartete Art`, true)
  }
...
async function project(files: Record<string, string>) {
  ...
  for (const [rel, content] of Object.entries(files)) await lumen.fs.writeFile(path.join(dir, rel), content)
