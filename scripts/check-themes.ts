...
expect(parseHex('#12345') === null, 'ungültige Farbe → null')
...
expect(Math.abs(white.l - 1) < 1e-4 && white.c < 1e-4, 'Weiß: L = 1, C = 0')
expect(Math.abs(black.l) < 1e-4, 'Schwarz: L = 0')
const out = oklchToHex({ l: 0.7, c: 0.4, h: 145 })
expect(parseHex(out) !== null, 'außerhalb des Gamuts wird auf sRGB begrenzt', out)
expect(Math.abs(hexToOklch(out)!.h - 145) < 2, 'Gamut-Begrenzung behält den Farbton', String(hexToOklch(out)!.h))

/* -------------------------------------------------------------- */
section('Kontrast (WCAG)')

expect(Math.abs(contrastRatio('#000000', '#ffffff') - 21) < 1e-6, 'Schwarz/Weiß = 21')
expect(Math.abs(contrastRatio('#777777', '#777777') - 1) < 1e-6, 'gleiche Farbe = 1')
expect(Math.abs(contrastRatio('#767676', '#ffffff') - 4.54) < 0.01, '#767676 auf Weiß ≈ 4.54')
expect(contrastRatio('#00000000', '#ffffff') < 1.001, 'transparent zählt als Hintergrund')
...
for (const [fg, bg, min] of [['#7c8cff', '#ffffff', 4.5], ['#333333', '#202020', 7], ['#fbbf24', '#fdf9f2', 3]] as const) {
  ...
  expect(hueShift < 6 || hexToOklch(fixed)!.c < 0.02, `ensureContrast behält den Farbton von ${fg}`, `${hueShift.toFixed(1)}°`)
}
expect(ensureContrast('#e4e7ec', '#0e1013', 4.5) === '#e4e7ec', 'ausreichender Kontrast bleibt unverändert')
...
for (const theme of themes) {
  ...
  expect(muted >= 4.5, `${theme.name}: gedämpfter Text ≥ 4.5`, muted.toFixed(2))
  ...
  expect(sameOrder, `${theme.name}: Reihenfolge der Flächenstufen bleibt`)
  ...
  expect(invertedDir === -originalDir, `${theme.name}: Flächen heben sich in die Gegenrichtung ab`)
  ...
  expect(accentBefore.c < 0.03 || hueDiff < 8, `${theme.name}: Akzent behält den Farbton`, `${hueDiff.toFixed(1)}°`)

  const back = invertTheme(inverted)
  const drift = Math.max(...(['bg', 'bgElevated', 'text', 'border'] as const).map((k) => deltaE(theme.ui[k], back.ui[k])))
  expect(drift < 0.05, `${theme.name}: zweimal invertiert ≈ Original (Flächen/Text)`, drift.toFixed(3))
}

/* -------------------------------------------------------------- */
section('Kontrastkorrektur und Paletten')

for (const theme of themes) {
  const broken: Theme = { ...theme, ui: { ...theme.ui, text: theme.ui.bgActive, textMuted: theme.ui.bg } }
  const issues = contrastIssues(broken)
  expect(issues.some((i) => i.key === 'ui:text'), `${theme.name}: kaputter Text wird gemeldet`)
  ...
  expect(contrastRatio(derived.ui.text, derived.ui.bg) >= 7, `${theme.name}: abgeleitete Flächen haben lesbaren Text`)
  ...
  expect(ordered, `${theme.name}: abgeleitete Flächenstufen geordnet`)
}
...
for (const [mode, matrix] of Object.entries(VISION_MATRICES)) {
  for (let row = 0; row < 3; row++) {
    ...
    expect(Math.abs(sum - 1) < 1e-3, `${mode}: Zeile ${row + 1} erhält Weiß`, sum.toFixed(4))
  }
  expect(simulateVision('#ffffff', mode as VisionMode) === '#ffffff', `${mode}: Weiß bleibt Weiß`)
...
expect(gray.slice(1, 3) === gray.slice(3, 5) && gray.slice(3, 5) === gray.slice(5, 7), 'Graustufen haben gleiche Kanäle', gray)
expect(deltaE(simulateVision('#ff0000', 'protanopia'), simulateVision('#00ff00', 'protanopia')) < deltaE('#ff0000', '#00ff00'), 'Protanopie nähert Rot und Grün an')

section('Auswahl im Editor')
{
  // `rgb(r g b / a)` — commas together with a slash the browser quietly discards.
  expect(hexToRgbChannels('#7c8cff') === '124 140 255', 'RGB-Kanäle ohne Kommas', hexToRgbChannels('#7c8cff'))
  expect(hexToRgbChannels('#7c8cff80') === '124 140 255', 'RGB-Kanäle ignorieren Alpha', hexToRgbChannels('#7c8cff80'))
  ...
  for (const ui of probes) {
    ...
    expect(Boolean(focused), `${ui.selection}: gültiges CSS`, colors.focused)
    expect(Boolean(parse(colors.inactive)), `${ui.selection}: gültiges CSS ohne Fokus`, colors.inactive)
    if (!focused) continue
