...
function main() {
  ...
  for (const file of files.sort()) {
    ...
    if (!EXEMPT.some((prefix) => file.startsWith(prefix))) {
      source.split('\n').forEach((raw, index) => {
        if (!DEVELOPER_TEXT.test(raw) || !isGerman(raw)) return
        hits.push({ line: index + 1, text: raw.trim() })
      })
    }
    if (hits.length) offenders.push({ file, hits: hits.sort((a, b) => a.line - b.line) })
  }
  ...
  for (const file of stale) process.stdout.write(`! ${file} is clean — remove it from scripts/docs-pending.txt\n`)
  ...
  if (done.length || stale.length) process.exit(1)
}
