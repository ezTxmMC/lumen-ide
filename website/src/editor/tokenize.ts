...
function identifierKind(word: string, before: string, after: string, brackets: string[]): TokenKind {
  ...
  if (before === '.' || before === '?.') return 'property'
  if (/^[A-Z]/.test(word) && /^[A-Z0-9_]+$/.test(word)) return 'constant'
  if (/^[A-Z]/.test(word)) return 'type'
  if (after.startsWith(':') && brackets.at(-1) === '{') return 'property'
  return 'variable'
}
...
export function tokenize(text: string): Line[] {
  ...
  while ((match = PATTERN.exec(text))) {
    ...
    if (comment) flat.push({ text: whole, kind: 'comment' })
    if (string) flat.push({ text: whole, kind: 'string' })
    if (number) flat.push({ text: whole, kind: 'number' })
    if (word) flat.push({ text: whole, kind: identifierKind(word, previous, rest, brackets) })
    if (operator) flat.push({ text: whole, kind: 'operator' })
    if (punctuation) {
      flat.push({ text: whole, kind: 'punctuation' })
      if ('([{'.includes(whole)) brackets.push(whole)
      if (')]}'.includes(whole)) brackets.pop()
    }
    if (other) flat.push({ text: whole, kind: null })
    if (!whole.trim()) {
...
export function indentOf(line: Line): number {
  const first = line[0]
  if (!first || first.kind !== null) return 0
