...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--token') options.token = argv[++i]
  ...
  if (!options.server) throw new Error('No server given (--server or LUMEN_EXT_SERVER)')
  if (!options.dryRun && !options.token) throw new Error('No token given (--token or LUMEN_EXT_TOKEN)')
...
async function newestPackages() {
  ...
  for (const name of files) {
    ...
    if (known && compareVersions(known.version, manifest.version) <= 0) continue
...
async function main() {
  ...
  if (!packages.length) throw new Error('No matching packages in extensions/dist')

  process.stdout.write(`Server: ${info.name} — ${options.server}\n\n`)
  if (options.dryRun) {
    for (const manifest of packages) process.stdout.write(`· ${manifest.id} ${manifest.version} (${manifest.name})\n`)
