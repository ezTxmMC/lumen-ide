...
test('status: branch header with upstream and ahead/behind', () => {
  const status = parseStatus(
    ['# branch.oid 1234567890abcdef', '# branch.head main', '# branch.upstream origin/main', '# branch.ab +2 -3', '# stash 4', ''].join(
      NUL,
    ),
  );
...
test('status: staged, unstaged, both, untracked, paths with spaces', () => {
  ...
  assert.deepEqual(
    status.staged.map((e) => [e.path, e.kind]),
    [
  ...
  assert.deepEqual(
    status.unstaged.map((e) => [e.path, e.kind]),
    [
      ['src/changed file.ts', 'modified'],
      ['both.txt', 'deleted'],
    ],
  );
  assert.deepEqual(
...
test('status: renames carry their original path', () => {
  const status = parseStatus(
    ['2 R. N... 100644 100644 100644 aaa bbb R100 new name.ts', 'old name.ts', '1 .M N... 100644 100644 100644 aaa bbb after.ts', ''].join(
      NUL,
    ),
  );
...
test('status: conflicts', () => {
  const status = parseStatus(
    ['u UU N... 100644 100644 100644 100644 a b c conflict.ts', 'u AA N... 000000 100644 100644 100644 a b c added both.ts', ''].join(NUL),
  );
  assert.deepEqual(
    status.conflicts.map((e) => [e.path, e.conflict]),
    [
      ['conflict.ts', 'bothModified'],
      ['added both.ts', 'bothAdded'],
    ],
  );
...
test('refs: local with tracking, remote without HEAD, tags', () => {
  ...
  assert.deepEqual(
    refs.remote.map((r) => [r.remote, r.branch]),
    [['origin', 'feature/x']],
  );
...
test('i18n: fallback to English works', () => {
  const t = createT({ locale: () => 'fr' });
  assert.equal(t('push.done', { branch: 'main' }), 'Pushed main.');
  assert.equal(createT({ locale: () => 'de' })('push.done', { branch: 'main' }), 'Pushed main.');
});
