...
export function createHistoryView({ ctx, repo, actions, t }) {
  function commitRow(commit) {
    ...
    return {
      ...
      actions: [
        { action: 'show', title: t('history.show'), icon: 'file-diff', payload: commit.hash },
      ],
      menu: [
  ...
  function render() {
    const { state } = repo
    if (!state.loaded) return { title: t('view.history'), nodes: [{ type: 'progress', label: t('busy.loading') }] }
    if (!state.isRepo) return { title: t('view.history'), nodes: [{ type: 'empty', title: t('empty.noRepo'), icon: 'history' }] }
    if (!state.log.length) return { title: t('view.history'), nodes: [{ type: 'empty', title: t('empty.noCommits'), hint: t('empty.noCommitsHint'), icon: 'git-commit' }] }
    ...
    return {
      ...
      nodes: [
        ...
        ...(more ? [{ type: 'buttons', buttons: [{ action: 'more', title: t('history.more'), icon: 'chevron-down', variant: 'secondary' }] }] : []),
      ],
    }
  }
