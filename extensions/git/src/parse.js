...
function addTracked(status, xy, path, orig) {
  const [x, y] = xy;
  if (x !== '.') {
    status.staged.push({ path, orig, status: x, kind: KIND[x] ?? 'modified' });
  }
  if (y !== '.') {
    status.unstaged.push({ path, orig, status: y, kind: KIND[y] ?? 'modified' });
  }
}
  ...
  for (let index = 0; index < records.length; index++) {
    ...
    if (type === 'u') {
      status.conflicts.push({
        ...
        status: 'U',
        kind: 'conflict',
        conflict: CONFLICT[parts[1]] ?? 'bothModified',
      });
    }
  }
  return status;
...
function parseRefs(decoration) {
  ...
  return decoration.split(', ').map((raw) => {
    ...
    if (text.startsWith('HEAD -> ')) {
      return { name: text.slice(8), kind: 'head' };
    }
    if (text === 'HEAD') {
      return { name: 'HEAD', kind: 'head' };
    }
    if (text.startsWith('tag: ')) {
      return { name: text.slice(5), kind: 'tag' };
    }
    if (text.includes('/')) {
      return { name: text, kind: 'remote' };
    }
...
export const REF_FORMAT =
  '%(refname)%1f%(refname:short)%1f%(objectname:short)%1f%(upstream:short)%1f%(upstream:track,nobracket)%1f%(HEAD)%1f%(committerdate:unix)%1f%(contents:subject)';
