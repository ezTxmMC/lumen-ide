...
export function runState(run) {
  if (run.status !== 'completed') return run.status === 'queued' || run.status === 'waiting' ? 'queued' : 'running'
  const byConclusion = { success: 'success', failure: 'failure', cancelled: 'cancelled', skipped: 'skipped', timed_out: 'failure', action_required: 'waiting', startup_failure: 'failure', neutral: 'skipped' }
...
export function createRunsView({ ctx, store, actions, t }) {
  function row(run) {
    ...
    return {
      ...
      tooltip: [`${run.name} #${run.run_number}`, t(`run.state.${state}`), run.head_sha?.slice(0, 8), run.html_url].filter(Boolean).join('\n'),
      icon: look.icon,
      iconTone: look.tone,
  ...
  function render() {
    ...
    if (blocked) return { title: t('view.runs'), toolbar, nodes: blocked }
