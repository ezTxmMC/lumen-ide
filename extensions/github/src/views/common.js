...
export function gate(store, slot, t) {
  ...
  if (!context) {
    return [{ type: 'progress', label: t('busy.loading') }];
  }
  const reasons = {
  ...
  if (error.kind === 'auth') {
    return [{ type: 'empty', title: t('empty.authFailed'), hint: error.message, icon: 'shield-alert', action: signIn }];
  }
  if (error.kind === 'rateLimit') {
    return [{ type: 'empty', title: t('empty.rateLimit'), hint: error.message, icon: 'timer' }];
  }
  return [
    {
      type: 'empty',
