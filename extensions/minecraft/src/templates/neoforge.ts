...
interface BuildParts {
  ...
  extension: string;
  version: string;
  parchmentBlock: string;
  datagen: boolean;
  data: string;
}

function neoforgeBuildKts({ values, plugin, extension, version, parchmentBlock, datagen, data }: BuildParts): string {
  return `plugins {
    \`java-library\`
    \`maven-publish\`
...
function neoforgeBuildGroovy({ values, plugin, extension, version, parchmentBlock, datagen, data }: BuildParts): string {
  return `plugins {
    id 'java-library'
    id 'maven-publish'
    id '${plugin}' version '${values.moddevVersion}'
...
function neoforgeBuild(values: FormValues): string {
  ...
  const parts: BuildParts = {
    values,
    datagen: isOn(values, 'datagen'),
    data: f.clientData ? 'clientData()' : 'data()',
    extension: legacy(values) ? 'legacyForge' : 'neoForge',
    plugin: legacy(values) ? 'net.neoforged.moddev.legacyforge' : 'net.neoforged.moddev',
    version: versionLine(legacy(values), kts),
    parchmentBlock: parchmentOn(values) ? parchmentLines(kts) : '',
  };
  return kts ? neoforgeBuildKts(parts) : neoforgeBuildGroovy(parts);
}
