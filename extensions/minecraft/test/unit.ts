...
await test('messages: every language has every key', () => {
  ...
  assert.deepEqual(Object.keys(MESSAGES).sort(), ['en', 'es', 'fr', 'it', 'nl', 'pl', 'pt']);
});
...
await test('translations: Spanish template names and badges', async () => {
  setLumen(fakeLumen({ net: offlineNet, language: 'es' }));
  assert.equal(template('minecraft-fabric').name, 'Mod de Fabric');
  setLumen(fakeLumen({ net: offlineNet }));
});
