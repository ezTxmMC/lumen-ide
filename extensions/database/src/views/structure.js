...
export function createStructureTab({ t, sessions, refresh, api }, { connection, schema, table }) {
  ...
  async function load() {
    ...
    try {
      ...
      if (!db) throw new Error(t('error.cancelled'))
  ...
  function render() {
    ...
    if (state.loading && !state.columns) nodes.push({ type: 'progress', label: t('table.loading') })
  ...
  return {
    ...
    async onAction({ action }) {
      ...
      if (action === 'data') api.openTable(connection, schema, table)
    },
  }
}
