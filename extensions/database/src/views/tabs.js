...
export function createTabs(deps) {
  ...
  const api = {
    ...
    refreshAll() {
      for (const instance of tabs.keys()) ctx.views.refresh(VIEW, instance)
    },
  }
  ...
  const provider = {
    render(instance) {
      ...
      if (!tab) return { nodes: [{ type: 'empty', icon: 'database', title: t('tab.gone'), hint: t('tab.goneHint') }] }
      return tab.render()
    },
    ...
    onClose(instance) {
      ...
      if (byKey.get(tab.key) === instance) byKey.delete(tab.key)
      tab.dispose?.()
    },
  }
