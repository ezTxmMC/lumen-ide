...
export function inlineParams(dialect, sql, params) {
  ...
  if (dialect === 'postgres') {
    return sql.replace(/\$(\d+)/g, (match, n) => (params[n - 1] !== undefined ? sqlLiteral(dialect, params[n - 1]) : match));
  }
  if (dialect === 'mssql') {
    return sql.replace(/@p(\d+)/g, (match, n) => (params[n - 1] !== undefined ? sqlLiteral(dialect, params[n - 1]) : match));
  }
  let index = 0;
...
function keyColumnSql(dialect, column) {
  if (column === ROWID.sqlite && dialect === 'sqlite') {
    return 'rowid';
  }
  if (column === ROWID.postgres && dialect === 'postgres') {
    return 'ctid';
  }
...
export function rowIdSelect(dialect) {
  if (dialect === 'sqlite') {
    return `rowid AS ${quoteIdent(dialect, ROWID.sqlite)}`;
  }
  if (dialect === 'postgres') {
    return `ctid::text AS ${quoteIdent(dialect, ROWID.postgres)}`;
  }
  return null;
}
...
export function insertStatement(dialect, { schema, table }, values) {
  ...
  if (!columns.length) {
    if (dialect === 'mysql') {
      return { sql: `INSERT INTO ${from} () VALUES ()`, params: [] };
    }
