...
export function setCell(pending, rowId, column, value, original) {
  ...
  if (Object.keys(edits).length) {
    pending.edits.set(rowId, edits);
  }
  if (!Object.keys(edits).length) {
    pending.edits.delete(rowId);
  }
}
...
export function pendingStatements(pending, { dialect, schema, table, keyOf }) {
  ...
  const statements = [];
  for (const rowId of pending.deletes) {
    statements.push(deleteStatement(dialect, target, keyOf(rowId)));
  }
  ...
  for (const row of pending.inserts) {
    statements.push(insertStatement(dialect, target, row.values));
  }
  return statements;
}
