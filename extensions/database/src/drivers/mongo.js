...
export function mongoUri(connection, password) {
  ...
  const auth = connection.user ? `${encodeURIComponent(connection.user)}${password ? `:${encodeURIComponent(password)}` : ''}@` : ''
...
export function parseDocument(text) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('A JSON object is expected')
  return value
}
