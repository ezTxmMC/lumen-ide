...
export function jsonSafe(value) {
  ...
  if (isBytes(value)) return toHex(value)
  if (Array.isArray(value)) return value.map(jsonSafe)
  if (typeof value === 'object') {
    if (typeof value.toHexString === 'function') return value.toHexString()
    if (typeof value.toJSON === 'function' && !(value.constructor === Object)) return jsonSafe(value.toJSON())
...
export function toCell(value) {
  ...
  if (isBytes(value)) return toHex(value, MAX_HEX_BYTES)
...
export function parseInput(text, column = {}, dialect = '') {
  ...
  if (isBinaryType(type) && /^0x([0-9a-f]{2})*$/i.test(text)) return Buffer.from(text.slice(2), 'hex')
  if (dialect === 'sqlite' && isNumericType(type) && /^-?\d+(\.\d+)?(e[+-]?\d+)?$/i.test(text.trim())) {
    const number = Number(text)
    if (Number.isSafeInteger(number) || !/^-?\d+$/.test(text.trim())) return number
...
export function toCsv(columns, rows, separator = ',') {
  ...
  for (const row of rows) lines.push(row.map((value) => csvField(value, separator)).join(separator))
  return `${lines.join('\r\n')}\r\n`
}
