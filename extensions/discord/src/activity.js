...
export function readSettings(values = {}) {
  ...
  return {
    ...
    privateProjects: String(raw.privateProjects ?? '').split(/\r?\n/).map((line) => line.trim()).filter(Boolean),
  }
}
...
export function repoUrlFromGitConfig(text) {
  ...
  for (const line of String(text ?? '').split(/\r?\n/)) {
    ...
    if (current && url && !current.url) current.url = url[1]
  }
...
export const assetKey = (id) => String(id).toLowerCase().replace(/[^a-z0-9_]/g, '_')
...
export function buildActivity(state, settings, t) {
  if (state.idle && settings.idleAction === 'clear') return null
  if (state.private && settings.privateMode === 'hide') return null
  ...
  if (state.file && !state.idle && showFile) activity.details = t('activity.editing', { file: baseName(state.file.path) })
  if (state.file && !state.idle && !showFile) activity.details = t('activity.editingHidden')

  const project = state.project?.name
  if (project && settings.showProject && !state.private) activity.state = t('activity.project', { project })
  if (project && settings.showProject && state.private) activity.state = t('activity.privateProject')
  if (settings.showElapsed && state.start) activity.timestamps = { start: state.start }
  ...
  if (!language || !settings.showLanguage || state.idle) return activity
  activity.assets = {
