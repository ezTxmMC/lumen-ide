...
await test('frames: encode and decode round trip, split over chunks', () => {
  ...
  const b = encodeFrame(OP_FRAME, { cmd: 'SET_ACTIVITY', text: 'Greetings' });
  ...
  assert.deepEqual(second.frames, [{ op: OP_FRAME, data: { cmd: 'SET_ACTIVITY', text: 'Greetings' } }]);
...
await test('sanitizeActivity: trims, truncates, drops short texts and bad buttons', () => {
  const activity = sanitizeActivity({
    details: '  Editing main.ts  ',
    state: 'x',
    extra: 1,
    timestamps: { start: 12.6 },
    assets: { large_image: 'c', large_text: 'a'.repeat(200), small_text: 'y' },
    buttons: [
      { label: 'Repo', url: 'https://example.com/a' },
      { label: 'Bad', url: 'file:///etc' },
      { label: 'Third', url: 'https://x.y' },
    ],
  });
...
await test('activity: each part can be switched off', () => {
  const activity = buildActivity(
    base,
    settings({ showFileName: 'false', showProject: 'false', showLanguage: 'false', showElapsed: 'false' }),
    t,
  );
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on('data', (chunk) => {
      ...
      for (const frame of frames) {
        ...
        if (onHandshake?.(socket, frame)) {
          continue;
        }
        socket.write(
          encodeFrame(OP_FRAME, { cmd: 'DISPATCH', evt: 'READY', data: { v: 1, user: { username: 'tester', global_name: 'Tester' } } }),
        );
      }
    });
  ...
  return new Promise((resolve) =>
    server.listen(file, () =>
      resolve({
        ...
        sockets,
        activities: () =>
          received.filter((frame) => frame.op === OP_FRAME && frame.data?.cmd === 'SET_ACTIVITY').map((frame) => frame.data.args.activity),
        close: () =>
          new Promise((done) => {
...
if (process.platform !== 'win32') {
  await test('client: handshake, READY, activity, throttle, ping, clearing', async () => {
    ...
    assert.deepEqual(
      states.map((status) => status.state),
  ...
  await test('client: without Discord it waits, retries and connects once Discord is there', async () => {
    ...
    for (const socket of discord.sockets) {
      socket.destroy();
    }
    await until(() => client.status().state !== 'connected');
    await until(() => client.status().state === 'connected');
