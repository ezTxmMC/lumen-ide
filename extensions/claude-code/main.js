...
function number(value) {
  ...
  if (!value || !Number.isFinite(parsed) || parsed <= 0) return undefined
...
function mcpFrom(value) {
  ...
  if (!servers || typeof servers !== 'object' || Array.isArray(servers)) throw new Error('MCP servers: expected an object of servers')
  return servers
}

/** The thinking setting: adaptive (the model decides), off, or a fixed budget. */
function thinkingFrom(settings) {
  if (settings.thinking === 'disabled') return { type: 'disabled' }
  if (settings.thinking === 'budget') {
    const budgetTokens = number(settings.thinkingBudget) ?? 8000
    return { type: 'enabled', budgetTokens }
  }
  if (settings.thinking === 'adaptive') return { type: 'adaptive' }
  return undefined
}

function settingSources(settings) {
  const sources = []
  if (settings.userSettings !== 'false') sources.push('user')
  if (settings.projectSettings !== 'false') sources.push('project')
  if (settings.localSettings !== 'false') sources.push('local')
  return sources
}
...
function flagSettings(settings) {
  const flags = {}
  if (settings.fastMode === 'true' || settings.fastMode === 'false') flags.fastMode = settings.fastMode === 'true'
  const style = String(settings.outputStyle ?? '').trim()
  if (style) flags.outputStyle = style
...
function streamDelta(event) {
  ...
  if (delta.type === 'text_delta' && delta.text) return [{ kind: 'delta', text: delta.text }]
  if (delta.type === 'thinking_delta' && delta.thinking) return [{ kind: 'delta', text: delta.thinking, thinking: true }]
  return []
}
...
const TRANSLATORS = {
  system(message) {
    ...
    if (message.subtype === 'status') return [{ kind: 'status', text: message.status ?? '' }]
    return []
  },
  ...
  assistant(message) {
    ...
    for (const block of message.message.content) {
      ...
      if (block.type === 'thinking' && block.thinking?.trim()) blocks.push({ type: 'thinking', text: block.thinking })
...
function optionsFor(request, abort, canUseTool) {
  ...
  for (const key of Object.keys(options)) {
    ...
    if (Array.isArray(options[key]) && !options[key].length) delete options[key]
  }
  return options
}
...
function nameOf(id) {
  ...
  if (!/^[a-z]+$/.test(parts[0]) || !parts.slice(1).every((part) => /^\d+$/.test(part))) return ''
...
function labelled(rows) {
  ...
  const label = (row) => {
    ...
    if (!name) return row.displayName || row.value
    return /\[1m\]$/i.test(row.value) || /\[1m\]$/i.test(row.resolvedModel ?? '') ? `${name} (1M)` : name
  }
  for (const row of rows) count.set(label(row), (count.get(label(row)) ?? 0) + 1)
...
function toModels(list) {
  ...
  if (!fallback || models.some((entry) => entry.isDefault)) return models
...
async function listModels({ settings = {}, refresh = false } = {}) {
  ...
  if (fresh && !refresh) return withExtras(modelCache.list)
...
const provider = {
  ...
  async models(options) {
    try {
      ...
      if (modelCache.list.length) return modelCache.list
