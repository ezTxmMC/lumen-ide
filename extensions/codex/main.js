...
function argsFor(request, images) {
  ...
  for (const override of lines(settings.configOverrides)) {
  ...
  if (settings.profile) {
    args.push('--config', `profile=${tomlString(settings.profile)}`);
  }
  args.push('--sandbox', sandboxFor(mode, settings));
  args.push('--cd', cwd);
  for (const dir of lines(settings.additionalDirectories)) {
  ...
  if (settings.hideReasoning === 'true') {
  ...
  if (APPROVALS.has(settings.approvalPolicy)) {
    args.push('--config', `approval_policy=${tomlString(settings.approvalPolicy)}`);
  }
  if (sessionId) {
...
function translate(event, context) {
  if (event.type === 'thread.started') {
    return [{ kind: 'session', sessionId: event.thread_id, model: context.model }];
  }
...
async function runTurn(request, emit) {
  ...
  try {
    ...
    for await (const line of reader) {
      ...
      for (const body of translate(event, context)) {
        emit(body);
      }
    }
